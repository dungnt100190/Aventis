...
Scenario: 01 Create a new task successfully
	...
	And count task in Meine Pendenzen/offene is offene (3)
	And count task in Erstellte Pendenzen/offene is offene (2)
	When Click search icon in Fällig field, choose option Ist gleich and input 01.11.2120
	...
	When User choose row 1 on Grid Task
...
Scenario: 03 Create new a task unsuccessfully when cancel 
	...
	And Pendenzen Detail area switches to view mode
	And Data of Task grid should be
	| Fällig     | Betreff          | Leistung | Fallträger               | Fallnummer | Person            | Ersteller                  | Empfänger                  | Status         | Erfasst    | Bearbeitung | Erledigt | Antwort               |
	...
	And count task in Meine Pendenzen/offene is offene (2)
	And count task in Erstellte Pendenzen/offene is offene (1)