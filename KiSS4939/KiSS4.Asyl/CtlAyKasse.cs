...
{
    public class CtlAyKasse : KissUserControl
    {
        private const string CLASSNAME = "CtlAyKasse";
        private KiSS4.Gui.KissButton btnAusbezahlt;
        ...
        private void btnAusbezahlt_Click(object sender, System.EventArgs e)
        {
            if (qryBeleg.Count == 0 || (int)qryBeleg["KbBuchungStatusCode"] != 2) return;

            if (!KissMsg.ShowQuestion(CLASSNAME, "BetragAusbezahlt", "Ist der Betrag von CHF {0} an {1} ausbezahlt worden?", 0, 0, true, string.Format("{0:n2}", qryBeleg["Betrag"]), qryBeleg["Person"]))
                return;
            ...
            qryBeleg_PositionChanged(this, EventArgs.Empty);
        }

        private void btnBelegDrucken_Click(object sender, System.EventArgs e)
        {
            if (qryBeleg.Count == 0) return;

            if ((int)qryBeleg["KbBuchungStatusCode"] != 2 &&
                !KissMsg.ShowQuestion(CLASSNAME, "BelegBereitsVerbucht", "Achtung: Dieser Beleg ist bereits verbucht. Soll er trotzdem ausgedruckt werden?"))
            {
                return;
            }
            ...
            prms[0] = new NamedPrm("KbBuchungID", qryBeleg["KbBuchungID"]);
            RepUtil.ExecuteReport("AyKassenbeleg", KissReportDestination.Viewer, prms);
        }
        ...
        private void editPersonX_UserModifiedFld(object sender, KiSS4.Gui.UserModifiedFldEventArgs e)
        {
            ...
            e.Cancel = !dlg.AyUnterstuetztePersonSuchen(editPersonX.Text, e.ButtonClicked);
        ...
        private void FrmAyKasse_Load(object sender, System.EventArgs e)
        {
        ...
        private void FrmAyKasse_Print(object sender, System.EventArgs e)
        {
            if (qryBeleg.Count == 0) return;

            if ((int)qryBeleg["KbBuchungStatusCode"] == 2)
            {
                KissMsg.ShowInfo(CLASSNAME, "BelegNochNichtAusbezahlt", "Der Beleg wurde noch nicht ausbezahlt (noch nicht verbucht)!");
                return;
            }

            GetKissMainForm().ContextPrint(this, "AyKassenbeleg");
        }

        private void FrmAyKasse_Search(object sender, System.EventArgs e)
        {
        ...
        {
            btnAusbezahlt.Enabled = ((int)qryBeleg["KbBuchungStatusCode"] == 2);
        }
        ...
        private void Suchen()
        {
            ...
            {
                KissMsg.ShowInfo(CLASSNAME, "Min1FeldAusgefuellt", "Mindestens eines der Felder 'Person', 'Verfalldatum von', 'Verfalldatum bis' muss ausgefüllt sein!");
                return;
            }

            string SqlWhere = " WHERE FAL.ModulID = 6 AND FLB.KbAuszahlungsartCode = 103 ";  //nur Kassenbelege
            ...
            qryBeleg.Fill(@"
				select distinct FAL.BaPersonID, FLB.*,
					FAL.FaFallID, SFP.BgFinanzplanID,
					...
					Verfalldatum = DATEADD(DAY, 5, FLB.ValutaDatum)
				from	FaLeistung FAL
					...
					inner join KbBuchung               FLB  on FLB.BgBudgetID = BGG.BgBudgetID
					inner join XUser                   USR  on USR.UserID = FAL.UserId
					left  join XUser                   KUSR on KUSR.UserID = FLB.BarbelegUserID  " +
                SqlWhere + @"

				order by Person, SortAusbezahlt, BelegDatum");

            while (qryBeleg.Count > 0 && !qryBeleg["KbBuchungStatusCode"].Equals(2) && qryBeleg.Next()) ;
        ...
        private void InitializeComponent()
        {
            ...
            this.btnAusbezahlt = new KiSS4.Gui.KissButton();
            ...
            // chkNichtAusbezahltX
            //
            this.chkNichtAusbezahltX.EditValue = false;
            ...
            this.chkGesperrtX.Location = new System.Drawing.Point(360, 113);
            ...
            this.btnFallInfo.Location = new System.Drawing.Point(514, 459);
            ...
            // btnAusbezahlt
            //
            this.btnAusbezahlt.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Left)));
            this.btnAusbezahlt.FlatStyle = System.Windows.Forms.FlatStyle.Flat;
            this.btnAusbezahlt.Location = new System.Drawing.Point(598, 459);
            this.btnAusbezahlt.Name = "btnAusbezahlt";
            this.btnAusbezahlt.Size = new System.Drawing.Size(94, 24);
            this.btnAusbezahlt.TabIndex = 56;
            this.btnAusbezahlt.Text = "ausbezahlt";
            this.btnAusbezahlt.UseCompatibleTextRendering = true;
            this.btnAusbezahlt.UseVisualStyleBackColor = false;
            this.btnAusbezahlt.Click += new System.EventHandler(this.btnAusbezahlt_Click);
            //
            // colAusbezahlt
            //
            ...
            this.Controls.Add(this.btnAusbezahlt);
            ...
            this.Controls.Add(this.btnBelegDrucken);
            ...
            this.Name = "CtlAyKasse";
            this.Text = "AY Kasse";
            this.AutoScaleMode = AutoScaleMode.Inherit;
            this.Search += new System.EventHandler(this.FrmAyKasse_Search);
            this.Load += new System.EventHandler(this.FrmAyKasse_Load);
