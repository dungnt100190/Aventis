...
namespace KiSS4.Sozialsystem.Famoz
{
    public class SozialsystemBuilder
    {
        ...
        #region Globale Konstanten für Grafische Darstellung
        ...
        List<SSGraphPerson> PersonList = new List<SSGraphPerson>();
        // Liste aller Relationen 
        List<SSGraphRelation> RelationList = new List<SSGraphRelation>();
        // Liste aller Linienstücke, welche für eine Relationslinie zusammen gesetzt werden

        List<SSGraphServiceProvider> ServProvList = new List<SSGraphServiceProvider>();
        List<SSGraphInvolvedPerson> InvPersList = new List<SSGraphInvolvedPerson>();
        List<SSGraphInvolOrg> InvOrgsList = new List<SSGraphInvolOrg>();

        List<BoxSpaces> aBoxSpacesListX = new List<BoxSpaces>();
        List<BoxSpaces> aBoxSpacesListY = new List<BoxSpaces>();
        ...
        {
            if (RelationList[IndexRel].IsProcessed) return;
        ...
        private System.Drawing.Point GetNewPersonPosition(int aPersonIdx, int aRelationIdx, bool DownWard)
        {
            ...
            int SymmStep = 1;
            if (RelationList[aRelationIdx].Symmetric) SymmStep = 0;
        ...
        private int GetIndex(int aPersonID)
        {
            ...
            for (int j = 0; j < PersonList.Count; j++)
                if (PersonList[j].PersonID == aPersonID) return j;
        ...
        private int GetNewPointOnLine(int LineY, int LineX, int WishedX) 
        {
            ...
            if (Anordnung == 1)
            {
                while (!(FreePositionFoundOnLeft && FreePositionFoundOnRight))
                {
                    ...
                    if (!FreePositionFoundOnLeft) FirstFreePositionOnLeft -= 1;
                    FreePositionFoundOnRight = PositionIsFree(FirstFreePositionOnRight, LineY);
                    if (!FreePositionFoundOnRight) FirstFreePositionOnRight += 1;
                }
                if (Math.Abs(FirstFreePositionOnLeft - LineX) < Math.Abs(FirstFreePositionOnRight - LineX)) return FirstFreePositionOnLeft;
                else return FirstFreePositionOnRight;
            }
            else //if (Anordnung == 2)
            {
        ...
        private bool PositionIsFree(int aX, int aY)
        {
            for (int l = 0; l < PersonList.Count; l++)
                if (PersonList[l].Location.X == aX &&
                    ...
                    PersonList[l].HasPosition) return false;
            return true;
        }
        ...
        private int GetDistanceLeftLineX(bool AddLineOnLeft, LineTypes LineType)
        {
            ...
            if (aBoxSpacesListX[0].LineCount > 0) MinDistX += cstBoxBorderX;
        ...
        private int GetDistanceTopLineY(int aRelIdx, int PersIdx, bool AddLineOnTop, LineTypes LineType, int newX1, int newX2)
        {
            ...
            else
            {
                ...
                if (HasIntersection) aBoxSpacesListY[0].LineCount += 1;
            }
            ...
            if (aBoxSpacesListY[0].LineCount > 1) MinDistY += ((aBoxSpacesListY[0].LineCount - 1) * cstLineSpaceY);
            ...
            else
            {
                ...
                ResultY = cstBoxBorderY + cstTitelLabelHeight;
                if (aBoxSpacesListY[0].LineCount > 1) ResultY += ((aBoxSpacesListY[0].LineCount - 1) * cstLineSpaceY);
                MoveTop = ResultY + 1;
            }
            Distance = MinDistY - aBoxSpacesListY[0].PixelSize;
            if (Distance > 0) MoveAllComponentsDown(0, MoveTop, Distance);
        ...
        private int GetNewHorizontalLinePositionX(int aLineIdx, bool AddNewLine, bool AddLineOnLeft, bool AddLabelLeft, bool AddLabelRight)
        {
            ...
            if (AddLabelLeft) aBoxSpacesListX[aLineIdx].HasLabelTopLeft = true;
            if (AddLabelRight) aBoxSpacesListX[aLineIdx].HasLabelBottomRight = true;

            // Linienplatz erhöhen
            if (AddNewLine) aBoxSpacesListX[aLineIdx].LineCount += 1;

            // Neue Grösse
            int NewWidth = 0;
            if (aBoxSpacesListX[aLineIdx].HasLabelTopLeft) NewWidth = cstLabelWidth; else NewWidth = cstBoxBorderX;
            if (aBoxSpacesListX[aLineIdx].HasLabelBottomRight) NewWidth += cstLabelWidth; else NewWidth += cstBoxBorderX;
            if (aBoxSpacesListX[aLineIdx].LineCount > 1) NewWidth += ((aBoxSpacesListX[aLineIdx].LineCount - 1) * cstLineSpaceX);

            // Neue Position
            int NewPosition = 0;
            if (aBoxSpacesListX[aLineIdx].HasLabelTopLeft) NewPosition = cstLabelWidth; else NewPosition = cstBoxBorderX;
            if (!AddLineOnLeft) NewPosition += ((aBoxSpacesListX[aLineIdx].LineCount - 1) * cstLineSpaceX);
            ...
            if (Distance > 0)
            {
                ...
                if (!AddLineOnLeft) aTop += NewPosition;
        ...
        private bool CheckIntersectionsY(int aRelIdx, int aLineIdx, int newX1, int newX2, int newY)
        {
            ...
            if (newX1 == 0 && newX2 == 0) return true;
            if (aRelIdx == -1) return true;
                    ...
                    (ClientLines[x].From.Y == newY && newY > 0))
            {
                ...
                if (!( (leftX < leftLine && rightX < leftLine) ||
                       (leftX > rightLine && rightX > rightLine) )) return true;
            }
            return false;
        }
        ...
        private int GetNewPositionY(int aLineIdx, bool AddLineOnTop)
        {
            ...
            if (aBoxSpacesListY[aLineIdx].HasLabelTopLeft) tmp = cstLabelHeight;
        ...
        private int GetNewVerticalLinePositionY2(int aRelIdx, int aLineIdx, bool AddNewLine, bool AddLineOnTop, bool AddLabelTop, bool AddLabelBottom, int newX1, int newX2)
        {
            ...
            // Angabe, ob bei dieser Linie der Platz für eine Beschriftung reserviert werden soll oder nicht
            if (AddLabelTop) aBoxSpacesListY[aLineIdx].HasLabelTopLeft = true;
            if (AddLabelBottom) aBoxSpacesListY[aLineIdx].HasLabelBottomRight = true;
            ...
            if (AddNewLine) 
            {
                ...
                else
                {
                    ...
                    if (HasIntersection) aBoxSpacesListY[aLineIdx].LineCount += 1;
                }
            }
            // alt: 30.09.2007 
            ...
            // Neue Grösse
            int NewHeight = 0;
            if (aBoxSpacesListY[aLineIdx].HasLabelTopLeft) NewHeight = cstLabelHeight; else NewHeight = cstBoxBorderY;
            if (aBoxSpacesListY[aLineIdx].HasLabelBottomRight) NewHeight += cstLabelHeight; else NewHeight += cstBoxBorderY;
            if (aBoxSpacesListY[aLineIdx].LineCount > 1) NewHeight += ((aBoxSpacesListY[aLineIdx].LineCount - 1) * cstLineSpaceY);
            

            // Neue Position
            int NewPosition = 0;
            if (aBoxSpacesListY[aLineIdx].HasLabelTopLeft) NewPosition = cstLabelHeight; else NewPosition = cstBoxBorderY;
            if ((!AddLineOnTop) && (aBoxSpacesListY[aLineIdx].LineCount>1)) NewPosition += ((aBoxSpacesListY[aLineIdx].LineCount - 1) * cstLineSpaceY);
            ...
            if (Distance > 0)
            {
                ...
                if (!AddLineOnTop) aTop += NewPosition;
        ...
        private int GetNewLineItem(int RelIdx, int FromPersIdx, int ToPersIdx, bool IsFirstItem, bool IsLastItem, LineDirectionType aLineDir, int LineIdxX, int LineIdxY) 
        {
            ...
            if (IsLastItem)
            {
                ...
                // für zusätzliche Abstände
                if (!IsFirstItem) ClientLines[MaxIdx].GridFrom = PersonList[aPersIdx].Location;
        ...
        private void MoveAllComponentsLeft(int aLine, int FromPos, int Distance)
        {
            ...
            {
                if (ClientLines[c].From.X > FromPos) ClientLines[c].From.X = ClientLines[c].From.X + Distance;
                if (ClientLines[c].To.X > FromPos) ClientLines[c].To.X = ClientLines[c].To.X + Distance;
            }
            for (int c = 0; c < InvPersLines.Count; c++)
            {
                if (!InvPersLines[c].IsStartPoint)
                    if (InvPersLines[c].From.X > FromPos) InvPersLines[c].From.X = InvPersLines[c].From.X + Distance;
                if (InvPersLines[c].To.X > FromPos) InvPersLines[c].To.X = InvPersLines[c].To.X + Distance;
            }
            for (int b = aLine + 1; b < aBoxSpacesListX.Count; b++) aBoxSpacesListX[b].StartTopLeft += Distance;
        }
        ...
        private void MoveAllComponentsDown(int aLine, int FromPos, int Distance)
        {
            ...
            for (int c = 0; c < ClientLines.Count; c++)
            {
                if (ClientLines[c].From.Y > FromPos) ClientLines[c].From.Y += Distance;
                if (ClientLines[c].To.Y > FromPos) ClientLines[c].To.Y += Distance;
            }
            for (int c = 0; c < InvPersLines.Count; c++) if (!InvPersLines[c].IsStartPoint)
            {
                if (!(InvPersLines[c].LineIndex == 1))
                    if (InvPersLines[c].From.Y > FromPos) InvPersLines[c].From.Y += Distance;
                if (InvPersLines[c].To.Y > FromPos) InvPersLines[c].To.Y += Distance;
            }
        for (int c = 0; c < InvOrgLines.Count; c++) if (!InvOrgLines[c].IsStartPoint)
            {
                if (!(InvOrgLines[c].LineIndex == 1))
                    if (InvOrgLines[c].From.Y > FromPos) InvOrgLines[c].From.Y += Distance;
                if (InvOrgLines[c].To.Y > FromPos) InvOrgLines[c].To.Y += Distance;
            }
            for (int b = aLine + 1; b < aBoxSpacesListY.Count; b++) aBoxSpacesListY[b].StartTopLeft += Distance;
        }
        ...
        private int GetNextClientBoxIndex_Vertical(int PersIdx, bool IsTop)
        {
            ...
            {
                if (IsTop && PersonList[x1].Location.Y == PersonList[PersIdx].Location.Y + 1) return x1;
                if (!IsTop && PersonList[x1].Location.Y == PersonList[PersIdx].Location.Y - 1) return x1;
            }
            return -1;
        }
        ...
        private bool DockingLineHasConflict(int PersIdx, bool IsTop, int DokPos, int actY)
        {
            ...
            if (actY == 0) return false;
            if (IsTop && PersonList[PersIdx].Location.Y == 0) return false;

            // Wenn keine Box existiert,  dann kein Konflikt
            int CheckPersIdx = GetNextClientBoxIndex_Vertical(PersIdx, IsTop);
            if (CheckPersIdx < 0) return false;
        ...
        private int CalculateDockingPosition(int PersIdx, DockPointTypes AType, bool PreferLeftSide, int actY)
        {
            ...
            if (AType.Equals(DockPointTypes.dpBottom))
            {
                ...
                if (PreferLeftSide) dpValue = dpMaxPosLeft; else dpValue = dpMaxPosRight;
                while (DockingLineHasConflict(PersIdx, true, dpValue, actY))
                {
                    if (PreferLeftSide) dpValue -= 1; else dpValue += 1;
                }
            }
            else
            {
 ...
 * */
        ...
        private int GetNewLineInvPersons(int InvPersIdx, int ClientIdx, bool IsStartLine, bool IsEndLine, LineDirectionType aLineDir)
        {
            ...
            if (IsStartLine) aPersLine.LineIndex = 0;
            if (!IsStartLine)
            {
                ...
                if (aLineDir.Equals(LineDirectionType.ldHor)) aPersLine.To.Y = aPersLine.From.Y; else aPersLine.To.X = aPersLine.From.X;
            }
        ...
        private int GetNewLineInvOrganisations(int InvOrgsIdx, int ClientIdx, bool IsStartLine, bool IsEndLine, LineDirectionType aLineDir)
        {
            ...
            else
            {
                ...
                if (aLineDir.Equals(LineDirectionType.ldHor)) aOrgsLine.To.Y = aOrgsLine.From.Y; 
                    else aOrgsLine.To.X = aOrgsLine.From.X;
            }
        ...
        private bool HasIntersection(SSGraphLineItem Line1, SSGraphLineItem Line2, bool IsSameSystem, out System.Drawing.Point NewCrossPnt)
        {
            ...
            if (IsSameSystem) HasPoint = (HasPoint && (Line1.RelationID != Line2.RelationID));
            return HasPoint;
        }
        ...
        public SozialsystemBuilder(CtlSozialsystem aCtlSozialsystem, int KlientID, int FallID)
        {

            #region SQL Anweisung um Daten aus Db zu holen
            // TODO: Für produktive Version hier noch korrigieren:
            // Zum Testen kann verwendet werden:
            //SqlCommand aCmd = new SqlCommand("exec [dbo].[spFaGetSozialsystem] 28808, 47245, NULL", Session.Connection);
            //SqlCommand aCmd = new SqlCommand("exec [dbo].[spFaGetSozialsystem] 100016, 49697, NULL", Session.Connection);

            // Korrekte Version:
            string cmd = string.Format("exec [dbo].[spFaGetSozialsystem] {0}, {1}, '{2}'", KlientID.ToString(), FallID.ToString(), DateTime.Now.ToString("yyyy.MM.dd"));
            SqlCommand aCmd = new SqlCommand(cmd, Session.Connection);
            #endregion
            ...
            for (int IdxR = 0; IdxR < RelationList.Count; IdxR++)
                if (!RelationList[IdxR].IsProcessed) ProcessRelation(IdxR);
            ...
            for (int i = 0; i < PersonList.Count; i++)
                ...
                {
                    if (MinX > PersonList[i].Location.X) MinX = PersonList[i].Location.X;
                    if (MinY > PersonList[i].Location.Y) MinY = PersonList[i].Location.Y;
                    if (MaxX < PersonList[i].Location.X) MaxX = PersonList[i].Location.X;
                    if (MaxY < PersonList[i].Location.Y) MaxY = PersonList[i].Location.Y;
                }
            if (MinX < 0)
            {
                // in X Richtung verschieben
                for (int i = 0; i < PersonList.Count; i++)
                    if (PersonList[i].HasPosition) PersonList[i].Location.X -= MinX;
                MaxX -= MinX;
            }
            if (MinY < 0)
            {
                ...
                for (int i = 0; i < PersonList.Count; i++)
                    if (PersonList[i].HasPosition) PersonList[i].Location.Y -= MinY;
                MaxY -= MinY;
            }
            #endregion
            ...
            for (int q = 0; q <= MaxX + 1; q++)
            {
                ...
                if ((q > 0) && (q < MaxX)) aSp.PixelSize += cstBoxBorderX;
            ...
            for (int q = 0; q <= MaxY + 1; q++)
            {
                ...
                if (q == 0) aSp.PixelSize += cstTitelLabelHeight;
                if ((q > 0) && (q < MaxX)) aSp.PixelSize += cstBoxBorderY;
                ...
                if (RelationList[z].LineType.Equals(LineTypes.ltVerMany))
            {
                ...
                WishedX = -1;
                if (RelationList[z].From.X > 0) WishedX = 1;
                DokPosX = GetDockingPointPosition(z, Idx1, NewLIdx, aDp,
                    RelationList[z].RelationName1, WishedX,
                    RelationList[z].LineType, WishedX, DockPointTextTypes.dptClient, 0);
                ClientLines[NewLIdx].From.X = PersonList[Idx1].Pixels.X + cstHalfSizeX + (DokPosX * cstLabelWidth);
                ...
                if (aDp.Equals(DockPointTypes.dpTop)) aDp = DockPointTypes.dpBottom; else aDp = DockPointTypes.dpTop;
                NewLIdx = GetNewLineItem(z, Idx1, Idx2, false, false, LineDirectionType.ldHor, -1, -1);
                // nicht notwendig: LineList[NewLIdx].To.Y = LineList[NewLIdx].From.Y;
                DokPosX = GetDockingPointPosition(z, Idx2, NewLIdx, aDp,
                    RelationList[z].RelationName2, WishedX,
                ...
                // nicht notwendig: LineList[NewLIdx].To.X = LineList[NewLIdx].From.X;
                ClientLines[NewLIdx].To.Y = PersonList[Idx2].Pixels.Y;
                if (!FromTopToBottom) ClientLines[NewLIdx].To.Y += cstClientBoxSizeY;
                ClientLines[NewLIdx].BoxPositionX = DokPosX;
            ...
            #region Involvierte Personen in Grafik einfügen
            CtlItemInvolvedPerson aInvPers;
            int NewPosInvPersY = cstTitelLabelHeight + cstBoxBorderY;
            NewPosInvPersY = aBoxSpacesListY[0].PixelSize;
            for (int Idx = 0; Idx < InvPersList.Count; Idx++)
            {
                aInvPers = new CtlItemInvolvedPerson(
                    InvPersList[Idx].FirstName + " " + InvPersList[Idx].LastName,
                    InvPersList[Idx].Gender, // Geschlecht
                    InvPersList[Idx].Function,
                    InvPersList[Idx].Contact,
                    InvPersList[Idx].ClientID);
                aCtlSozialsystem.panInvolvedPersons.Controls.Add(aInvPers);
                aInvPers.Left = cstBordInvPers;
                aInvPers.Top = NewPosInvPersY;

                NewPosInvPersY = NewPosInvPersY + cstDistanceInvPersY + aInvPers.Height;
                // Für Linien Berechnung:
                InvPersList[Idx].Top = aInvPers.Top;
                InvPersList[Idx].Left = aInvPers.Left;
                InvPersList[Idx].Height = aInvPers.Height;
                InvPersList[Idx].Width = aInvPers.Width;
            }
            #endregion

            #region Linien für Involvierte Personen erstellen
            //
            bool DrawLinesInvPersons = false;
            int DokPosY;
            //int DiffX = aCtlSozialsystem.panInvolvedPersons.Width;
            int DiffX = 0;
            int DiffY = cstTitelLabelHeight + cstDistanceInvPersY;
            DiffY = 0;
            int InvPersIndex = 0;

            if (DrawLinesInvPersons)
            {
                //for (int Idx = 0; Idx < InvPersList.Count; Idx++) if (InvPersList[Idx].ClientID > 0)
                for (int Idx = InvPersList.Count - 1; Idx > -1; Idx--) if (InvPersList[Idx].ClientID > 0)
                    {
                        int PersIdx = GetIndex(InvPersList[Idx].ClientID);
                        if (PersIdx >= 0)
                        {
                            ...
                            NewPosX = GetDistanceLeftLineX(true, LineTypes.ltNone);

                            // Neue Linie erstellen
                            // Für alle gehen wir zuerst nach rechts
                            InvPersIndex += 1;
                            NewLIdx = GetNewLineInvPersons(InvPersIndex, PersIdx, true, false, LineDirectionType.ldHor);
                            InvPersLines[NewLIdx].From.X = InvPersList[Idx].Left + InvPersList[Idx].Width - DiffX;
                            InvPersLines[NewLIdx].To.X = NewPosX;
                            //PersLineList[NewLIdx].From.Y = InvPersList[Idx].Top + (InvPersList[Idx].Height / 2) - DiffY;
                            InvPersLines[NewLIdx].From.Y = InvPersList[Idx].Top + (InvPersList[Idx].Height / 2);
                            InvPersLines[NewLIdx].To.Y = InvPersLines[NewLIdx].From.Y;

                            aDp = DockPointTypes.dpLeft;
                            if (PersonList[PersIdx].Location.X > 0)
                            {
                                // schauen, ob die Linie horizontal direkt zur Box des Klienten geführt werden,
                                ...
                                if (HasClientsOnHorLine(ToY, 0, ToX2))
                                {
                                    ...
                                    aDp = DockPointTypes.dpBottom;
                                    if (InvPersLines[NewLIdx].To.Y < ToY) aDp = DockPointTypes.dpTop;
                                }
                            }

                            // jetzt vertikale Linie zeichnen
                            if (aDp.Equals(DockPointTypes.dpLeft))
                            {
                                // Es kann links an der Box angedockt werden
                                NewLIdx = GetNewLineInvPersons(InvPersIndex, PersIdx, false, false, LineDirectionType.ldVer);
                                InvPersLines[NewLIdx].To.Y = PersonList[PersIdx].Pixels.Y + cstHalfSizeY;

                                // jetzt noch die horizontale Linie erstellen und abschliessen
                                NewLIdx = GetNewLineInvPersons(InvPersIndex, PersIdx, false, true, LineDirectionType.ldHor);
                                InvPersLines[NewLIdx].To.X = PersonList[PersIdx].Pixels.X;
                            }
                            else
                            {
                                ...
                                NewLIdx = GetNewLineInvPersons(InvPersIndex, PersIdx, false, false, LineDirectionType.ldVer);
                                InvPersLines[NewLIdx].To.Y = NewPosY;

                                // jetzt horizontal nach rechts gehen, damit an die Box von oben oder unten angedockt werden kann
                                NewLIdx = GetNewLineInvPersons(InvPersIndex, PersIdx, false, false, LineDirectionType.ldHor);
                                WishedX = -4;
                                DokPosX = GetDockingPointPosition(Idx, PersIdx, NewLIdx, aDp, "", WishedX, LineTypes.ltNone, WishedX, DockPointTextTypes.dptInvPerson, 0);
                                InvPersLines[NewLIdx].To.X = PersonList[PersIdx].Pixels.X + cstHalfSizeX + (DokPosX * cstLabelWidth);

                                // dann vertikal andocken
                                NewLIdx = GetNewLineInvPersons(InvPersIndex, PersIdx, false, true, LineDirectionType.ldVer);
                                InvPersLines[NewLIdx].To.Y = PersonList[PersIdx].Pixels.Y;
                                if (aDp.Equals(DockPointTypes.dpBottom)) InvPersLines[NewLIdx].To.Y += cstClientBoxSizeY;
                            }
                        }
                    }
            }
            ...
            CtlItemInvolvedOrganisation aInvOrg;
            ...
            {
                aInvOrg = new CtlItemInvolvedOrganisation(
            ...
            DiffX = aCtlSozialsystem.panInvolvedPersons.Width;
            //DiffX = 0;
            DiffY = cstTitelLabelHeight;
            DiffY = 0;
            int InstIndex = 0;
            bool DrawLinesInvOrgs = false;

            if (DrawLinesInvOrgs)
            {
                for (int Idx = 0; Idx < InvOrgsList.Count; Idx++) if (InvOrgsList[Idx].ClientID > 0)
                    {
                        ...
                        if (PersIdx >= 0)
                        {
                            ...
                            if (PersonList[PersIdx].Location.X < MaxX)
                            {
                                ...
                                if (HasClientsOnHorLine(ToY, 0, ToX2))
                                {
                                    ...
                                    // je nachdem, wo der Weg kleiner ist
                                    aDp = DockPointTypes.dpBottom;
                                    if (InvOrgLines[NewLIdx].To.Y < ToY) aDp = DockPointTypes.dpTop;
                                }
                            }
                            ...
                            else
                            {
                                ...
                                if (aDp.Equals(DockPointTypes.dpBottom)) InvOrgLines[NewLIdx].To.Y += cstClientBoxSizeY;
                            }
                        }
                    }
            }
            ...
            CtlItemServiceProvider aServProv;
            int ServProvWidth = 0;
            for (int Idx = 0; Idx < ServProvList.Count; Idx++)
            {
                aServProv = new CtlItemServiceProvider(
            ...
            for (int i = 0; i < InvPersLines.Count; i++)
            {
                if (TotalMaxX < InvPersLines[i].From.X) TotalMaxX = InvPersLines[i].From.X;
                if (TotalMaxY < InvPersLines[i].From.Y) TotalMaxY = InvPersLines[i].From.Y;
                if (TotalMaxX < InvPersLines[i].To.X) TotalMaxX = InvPersLines[i].To.X;
                if (TotalMaxY < InvPersLines[i].To.Y) TotalMaxY = InvPersLines[i].To.Y;
            }
            ...
            for (int i = 0; i < InvOrgLines.Count; i++)
            {
                ...
                {
                    if (TotalMaxX < InvOrgLines[i].From.X + cstBoxBorderX) TotalMaxX = InvOrgLines[i].From.X + cstBoxBorderX;
                    if (TotalMaxY < InvOrgLines[i].From.Y + cstBoxBorderY) TotalMaxY = InvOrgLines[i].From.Y + cstBoxBorderY;
                    if (TotalMaxX < InvOrgLines[i].To.X + cstBoxBorderX) TotalMaxX = InvOrgLines[i].To.X + cstBoxBorderX;
                    if (TotalMaxY < InvOrgLines[i].To.Y + cstBoxBorderY) TotalMaxY = InvOrgLines[i].To.Y + cstBoxBorderY;
                }
            }
            for (int i = 0; i < InvOrgLines.Count; i++)
            {
                if (InvOrgLines[i].IsStartPoint) InvOrgLines[i].From.X = TotalMaxX + cstBordInvOrgs;
            }
            ...
            //
            for (int i = 0; i < PersonList.Count; i++) PersonList[i].dpListBottom.Clear();
            for (int i = 0; i < PersonList.Count; i++) PersonList[i].dpListTop.Clear();
            for (int i = 0; i < PersonList.Count; i++) PersonList[i].dpListRight.Clear();
            for (int i = 0; i < PersonList.Count; i++) PersonList[i].dpListLeft.Clear();
            PersonList.Clear();
            RelationList.Clear();
            ServProvList.Clear();
            InvPersList.Clear();
            ...
            for (int i = 0; i < ClientLines.Count; i++) ClientLines[i].CrossPoints.Clear();
            ClientLines.Clear();
            for (int i = 0; i < InvPersLines.Count; i++) InvPersLines[i].CrossPoints.Clear();
            InvPersLines.Clear();
            for (int i = 0; i < InvOrgLines.Count; i++) InvOrgLines[i].CrossPoints.Clear();
            InvOrgLines.Clear();
