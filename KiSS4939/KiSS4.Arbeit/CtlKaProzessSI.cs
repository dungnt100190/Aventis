...
{
    public class CtlKaProzessSI : KiSS4.Gui.KissUserControl
    {
        #region Fields
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(CtlKaProzessSI));
        ...
        {
            if (KissMsg.ShowQuestion("CtlKaProzessSI", "FallWiedereroeffnen", "Soll der geschlossene Fall wieder geöffnet werden ?"))
            {
        ...
        private void qryFaLeistung_BeforePost(object sender, System.EventArgs e)
        {
            ...
            if (!DBUtil.IsEmpty(qryFaLeistung["DatumBis"]))
            {
                ...
                // all fields have a value in "Vermittlung SI Übersicht"?
                cnt = Convert.ToInt32(DBUtil.ExecuteScalarSQL(@"SELECT COUNT(*)
                                    FROM KaVermittlungSI
                ...
                if (cnt > 0)
                {
                    ...
                    if (cnt > 0)
                    {
                        datDatumBis.EditValue = null;
                        qryFaLeistung["DatumBis"] = DBNull.Value;
                        throw new KissInfoException("Abschluss nicht möglich!\r\nNicht alle Einsätze korrekt ausgefüllt in Vermittlung SI Einsätze\r\n(Datum Einsatzende, durch, Grund)");
                    }
                }
        ...
        private void SetEditMode()
        {
            ...
            if (Session.User.IsUserAdmin || (int) qryFaLeistung["UserID"] == Session.User.UserID || DBUtil.UserHasRight("CtlKaProzessSI"))
            {
                bool open = DBUtil.IsEmpty(qryFaLeistung["DatumBis"]);
                bool archived = !DBUtil.IsEmpty(qryFaLeistung["FaLeistungArchivID"]);

                qryFaLeistung.CanUpdate = DBUtil.UserHasRight("CtlKaProzessSI", "UI");
