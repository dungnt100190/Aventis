...
{
    public class CtlKaProzessInizio : KiSS4.Gui.KissUserControl
    {
        #region Fields
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(CtlKaProzessInizio));
        ...
        {
            if (KissMsg.ShowQuestion("CtlKaProzessInizio", "FallWiedereroeffnen", "Soll der geschlossene Fall wieder geöffnet werden ?"))
            {
            ...
            if (!DBUtil.IsEmpty(qryFaLeistung["DatumBis"]))
            {
                ...
                cnt = Convert.ToInt32(DBUtil.ExecuteScalarSQL(@"
                    ...
                    FROM KaVermittlungEinsatz VEI
                        LEFT JOIN KaVermittlungVorschlag VOR ON VOR.KaVermittlungVorschlagID = VEI.KaVermittlungVorschlagID
                        LEFT JOIN KaInizio INI ON INI.FaLeistungID = VOR.FaLeistungID
                    WHERE VOR.FaLeistungID = {0}
                    AND VEI.InizioEinsatzAbbruchDurchCode IS NULL
                    AND VEI.InizioAnschlussloesungCode IS NULL
                    AND VOR.VorschlagResultat = 1
                    AND (INI.AbschlussPhaseCode IS NULL OR INI.AbschlussPhaseCode NOT IN (1,3))"
                    , FaLeistungID));
                ...
                if (cnt > 0)
                {
                    ...
                    qryFaLeistung["DatumBis"] = DBNull.Value;
                    throw new KissInfoException("Abschluss nicht möglich!\r\n'Abbruch durch', 'Anschlusslösung' oder 'Abschluss Phase1' nicht ausgefüllt!");
                }
        ...
        private void SetEditMode()
        {
            ...
            if (Session.User.IsUserAdmin || (int)qryFaLeistung["UserID"] == Session.User.UserID || DBUtil.UserHasRight("CtlKaProzessInizio", "U"))
            {
                bool open = DBUtil.IsEmpty(qryFaLeistung["DatumBis"]);
                bool archived = !DBUtil.IsEmpty(qryFaLeistung["FaLeistungArchivID"]);

                qryFaLeistung.CanUpdate = DBUtil.UserHasRight("CtlKaProzessInizio", "U");
