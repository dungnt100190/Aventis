...
=================================================================================================
  Description
-------------------------------------------------------------------------------------------------
  SUMMARY: Use this script to migrate table Hist_BaPerson.

  TODO: add or remove columns in INSERT and SELECT statement
=================================================================================================*/

--*****************************************************************************
-- Migrate table "Hist_BaPerson"
--*****************************************************************************
PRINT ('Info: **** Hist_BaPerson **** Start');
GO
...
-------------------------------------------------------------------------------
-- Cleanup invalid data in Hist_BaPerson
-------------------------------------------------------------------------------
PRINT ('Info: Cleanup invalid data in table "Hist_BaPerson"');
GO

-- remove invalid entries
-- <none>

-- PRINT ('Info: Deleted "' + CONVERT(VARCHAR(20), @@ROWCOUNT) + '" entries in table "Hist_BaPerson"');
-- GO

-------------------------------------------------------------------------------
-- Drop Triggers
-------------------------------------------------------------------------------
PRINT ('Info: Delete triggers of table "Hist_BaPerson"');
GO
...
-------------------------------------------------------------------------------
-- info
PRINT ('Info: Auto dropping FKs of other tables pointing to table "Hist_BaPerson"');
GO
...
INSERT INTO #TmpNoCheckFK (Name)
  ...
  WHERE OBJECT_NAME(parent_object_id) = 'Hist_BaPerson'
    AND is_not_trusted = 1;

-------------------------------------------------------------------------------
-- Drop depending objects such as keys and constraints of table Hist_BaPerson
-------------------------------------------------------------------------------
PRINT ('Info: Auto dropping PK, IXs, UKs, FKs, DFs and CKs of table "Hist_BaPerson"');
GO
...
-------------------------------------------------------------------------------
-- rename original table "Hist_BaPerson" to "_Old_Hist_BaPerson"
-------------------------------------------------------------------------------
PRINT ('Info: Rename original table "Hist_BaPerson" to "_Old_Hist_BaPerson".');
GO

-- rename table to _old_...
EXEC dbo.spDropObject _Old_Hist_BaPerson;
EXECUTE sp_rename N'Hist_BaPerson', N'_Old_Hist_BaPerson', 'OBJECT';
GO

-------------------------------------------------------------------------------
-- recreate table "Hist_BaPerson"
-------------------------------------------------------------------------------
PRINT ('Info: Recreate new table "Hist_BaPerson".');
GO

-- create new table from definition
{Include:Hist_BaPerson}
GO

-------------------------------------------------------------------------------
-- disable history-trigger
-------------------------------------------------------------------------------
PRINT ('Info: Disable history trigger on table "Hist_BaPerson".');
GO

-- disable trigger (remember to enable trigger again!!)
-- optional: DISABLE TRIGGER trHist_Hist_BaPerson ON Hist_BaPerson;
GO

-------------------------------------------------------------------------------
-- disable NOCHECK-FKs
-------------------------------------------------------------------------------
PRINT ('Info: Disable NOCHECK FKs on table "Hist_BaPerson".');
GO
...
WHILE (@EntriesIterator <= @EntriesCount)
BEGIN
  SELECT @SQL = 'ALTER TABLE dbo.Hist_BaPerson NOCHECK CONSTRAINT [' + TMP.name + '];'
...
-------------------------------------------------------------------------------
PRINT ('Info: Start with migration of data from old to new table "Hist_BaPerson".');
GO
...
BEGIN TRY
  -- migrate data (CUSTOM)
  SET IDENTITY_INSERT dbo.Hist_BaPerson ON;
  
  -- do reimport
  IF (NOT EXISTS (SELECT TOP 1 1 
                  FROM dbo._Old_Hist_BaPerson))
  BEGIN
  ...
  BEGIN
    ...
    -- TODO: add or remove columns in INSERT and SELECT statement
    EXEC('
    INSERT INTO dbo.Hist_BaPerson (BaPersonID,
                                   StatusPersonCode,
                                   Titel,
                                   Name,
                                   FruehererName,
      ...
      SELECT BaPersonID                         = OLD.BaPersonID,
             ...
             VerID_DELETED                      = OLD.VerID_DELETED
      FROM dbo._Old_Hist_BaPerson OLD WITH (HOLDLOCK TABLOCKX);

    -- info
    PRINT (''Info: Migrated "'' + CONVERT(VARCHAR(20), @@ROWCOUNT) + ''" entries in table "Hist_BaPerson"'');
    ');
    
  END;
  
  SET IDENTITY_INSERT dbo.Hist_BaPerson OFF;
  
  -- remember to remove old table in next release!
  -- >: EXEC dbo.spDropObject _Old_Hist_BaPerson;
  
  PRINT ('Info: Done migrating data to new table "Hist_BaPerson".');
END TRY
BEGIN CATCH
  ...
  RAISERROR ('Critical Error: Could not migrate data into new table "Hist_BaPerson". Database error was: %s', 18, 1, @ErrorMessage);
END CATCH;
GO
...
-------------------------------------------------------------------------------
PRINT ('Info: Enable history trigger on table "Hist_BaPerson".');
GO

-- enable trigger
-- optional: ENABLE TRIGGER trHist_Hist_BaPerson ON Hist_BaPerson;
GO

-------------------------------------------------------------------------------
-- Recreate FKs on other tables
-------------------------------------------------------------------------------
PRINT ('Info: Auto recreating FKs of other tables pointing to table "Hist_BaPerson"');
GO
...
-------------------------------------------------------------------------------
PRINT ('Info: Reenable NOCHECK FKs on table "Hist_BaPerson".');
GO
...
WHILE (@EntriesIterator <= @EntriesCount)
BEGIN
  SELECT @SQL = 'ALTER TABLE dbo.Hist_BaPerson CHECK CONSTRAINT [' + TMP.name + '];'
  FROM #TmpNoCheckFK TMP
  WHERE TMP.ID = @EntriesIterator;
