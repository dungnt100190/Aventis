...
IF (EXISTS (SELECT TOP 1 1 
            ...
            WHERE name = N'{DocDBName}'
              AND state_desc = N'RESTORING')) 
BEGIN
  PRINT ('Info: Database "{DocDBName}" is currently in RESTORE state and will be droped now to prevent error on ALTER statements.');
  DROP DATABASE [{DocDBName}];
END;
GO
...
BEGIN
  ...
  ALTER DATABASE [{DocDBName}] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
END;
GO
...
SELECT @BackupFile = CASE 
                     ...
                     END,
       @IsDoc      = 1, -- DocDB
       @DBPath     = N'{DBRootDirectory}\' + @DBName;
...
USE [master];
ALTER DATABASE [{DocDBName}] SET MULTI_USER WITH NO_WAIT;
GO

-- use current database
PRINT ('Info: Switch to database [{DocDBName}] now: ' + CONVERT(VARCHAR, GETDATE(), 113));
USE [{DocDBName}];
GO