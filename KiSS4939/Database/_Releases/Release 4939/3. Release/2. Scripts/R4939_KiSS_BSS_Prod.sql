...
************************************************************
  Settings:
    ...
    databaseName: [KiSS4_P]
    databaseName document: [KiSS4_DOC_P]
...
IF (DB_ID('KiSS4_P') IS NULL)
BEGIN
  RAISERROR('The Database ''KiSS4_P'' does not exist on the server!', 20, -1) WITH LOG;
END;
GO
 ...
 * #4932: Dokumenten DB-Name mit KiSS4_P_DOC ersetzen für das neue
 * Release-Tool
 * 
...
FROM KiSS4_DOC_P.dbo.XDocument
--@@TODO ???
...
BEGIN
  ...
  -- create login
  CREATE LOGIN [kiss_bm] WITH PASSWORD=N'textmarken88$', DEFAULT_DATABASE=[KiSS4_P], CHECK_EXPIRATION=OFF;
END
ELSE
BEGIN
  -- info
...
-- check if database exists
IF (DB_ID('KiSS4_DOC_P') IS NULL)
BEGIN
  RAISERROR('The Database ''KiSS4_DOC_P'' does not exist on the server!', 20, -1) WITH LOG;
END;
GO
...
USE [msdb]
GO
/****** Object:  Job [KiSS4_P_Pendenzen]    Script Date: 02/11/2009 09:33:40 ******/
DECLARE @job_id varchar(100)

SELECT @job_id = job_id FROM msdb.dbo.sysjobs_view WHERE name = N'KiSS4_P_Pendenzen'

IF  @job_id IS NOT NULL
  EXEC msdb.dbo.sp_delete_job @job_id, @delete_unused_schedule=1
GO
 
/****** Object:  Job [KiSS4_P_Pendenzen]    Script Date: 02/11/2009 09:31:53 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'KiSS4_P_Pendenzen', 
		@enabled= @enableJob, 
...
/*
************************************************************
  PROCESSED 158 FILES IN 00:00:07.8470964
************************************************************
*/
PRINT ('
************************************************************
