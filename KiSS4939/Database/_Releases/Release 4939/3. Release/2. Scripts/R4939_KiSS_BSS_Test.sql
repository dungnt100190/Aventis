...
************************************************************
  Settings:
    ...
    databaseName: [KiSS_PlausEx_central]
    databaseName document: [KiSS_PlausEx_central_DOC]
...
IF (DB_ID('KiSS_PlausEx_central') IS NULL)
BEGIN
  RAISERROR('The Database ''KiSS_PlausEx_central'' does not exist on the server!', 20, -1) WITH LOG;
END;
GO
 ...
 * #4932: Dokumenten DB-Name mit KiSS_PlausEx_central_DOC ersetzen für das neue
 * Release-Tool
 * 
...
FROM KiSS_PlausEx_central_DOC.dbo.XDocument
--@@TODO ???
...
IF (EXISTS (SELECT TOP 1 1
            ...
            WHERE name = @RoleNameBookmarksOnly
              AND type = 'R'
              AND owning_principal_id = USER_ID('kiss')))
BEGIN
...
BEGIN
  -- drop it
  EXEC sp_droprolemember 'db_owner', 'kiss';

  -- info
  PRINT ('Info: Removed user "kiss" from db_owner rights members');
END
ELSE
BEGIN
  -- info
  PRINT ('Info: User "kiss" is not member of role "db_owner"');
END;
...
BEGIN
  ...
  -- info
  PRINT ('Info: Drop assignment of user "kiss"');
...
CREATE USER [kiss] FOR LOGIN [kiss] WITH DEFAULT_SCHEMA = [dbo];

-- info
PRINT ('Info: Created user "kiss" for login on database');

IF (ISNULL(@UserSchemaName, '') <> '')
  BEGIN
    -- info
    PRINT ('Info: User owns a schema, rechanging owner of schema "' + @UserSchemaName + '" with owner [dbo] back to "kiss".');
     
    -- rechange owner of schema to SqlUserName
    SET @SqlStatement = N'ALTER AUTHORIZATION ON SCHEMA::[' + @UserSchemaName + N'] TO [kiss];';
    EXEC sp_executesql @SqlStatement;
  END;
...
---------------------------------------
-- public roles
EXEC sp_addrolemember 'db_datareader', 'kiss';
EXEC sp_addrolemember 'db_datawriter', 'kiss';

-- KiSS roles
EXEC sp_addrolemember @RoleNameExecutor, 'kiss';
EXEC sp_addrolemember @RoleNameDefinitionViewer, 'kiss';

-- add required/restrict rights (e.g. for INDENTITY_INSERTs)
GRANT ALTER ANY SCHEMA TO [kiss];
DENY CREATE SCHEMA TO [kiss];

USE [master];
GO
BEGIN TRY
  GRANT EXECUTE ON sys.xp_msver TO [kiss];
END TRY
BEGIN CATCH
  PRINT ('Warning: Could not grant rights on sys.xp_msver to kiss!')
END CATCH;
GO
USE [KiSS_PlausEx_central];
GO
...
-- info
PRINT ('Info: Added user "kiss" as member to specific roles');
PRINT ('');
GO
...
BEGIN
  ...
  -- create login
  CREATE LOGIN [kiss_bm] WITH PASSWORD=N'textmarken88$', DEFAULT_DATABASE=[KiSS_PlausEx_central], CHECK_EXPIRATION=OFF;
END
ELSE
BEGIN
  -- info
...
-- check if database exists
IF (DB_ID('KiSS_PlausEx_central_DOC') IS NULL)
BEGIN
  RAISERROR('The Database ''KiSS_PlausEx_central_DOC'' does not exist on the server!', 20, -1) WITH LOG;
END;
GO
...
USE [msdb]
GO
/****** Object:  Job [KiSS_PlausEx_central_Pendenzen]    Script Date: 02/11/2009 09:33:40 ******/
DECLARE @job_id varchar(100)

SELECT @job_id = job_id FROM msdb.dbo.sysjobs_view WHERE name = N'KiSS_PlausEx_central_Pendenzen'

IF  @job_id IS NOT NULL
  EXEC msdb.dbo.sp_delete_job @job_id, @delete_unused_schedule=1
GO
 
/****** Object:  Job [KiSS_PlausEx_central_Pendenzen]    Script Date: 02/11/2009 09:31:53 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'KiSS_PlausEx_central_Pendenzen', 
		@enabled= @enableJob, 
		...
		@owner_login_name=N'kiss', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Pendenzen anlegen', 
		...
		@database_name=N'KiSS_PlausEx_central', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
INSERT  INTO dbo.KesMassnahme (KesMassnahmeID, FaLeistungID, IsKS, DocumentID_Errichtungsbeschluss, DocumentID_Aufhebungsbeschluss, DatumVon, DatumBis, KesAufgabenbereichCodes, KesIndikationCodes, Auftragstext, KesElterlicheSorgeObhutCode_ElterlicheSorge, KesElterlicheSorgeObhutCode_Obhut, FuersorgerischeUnterbringung, Platzierung_BaInstitutionID, KesGrundlageCode, UebernahmeVon_Datum, UebernahmeVon_OrtschaftCode, UebernahmeVon_PLZ, UebernahmeVon_Ort, UebernahmeVon_Kanton, UebernahmeVon_KesBehoerdeID, AenderungVom_Datum, KesAenderungsgrundCode, UebertragungAn_Datum, UebertragungAn_OrtschaftCode, UebertragungAn_PLZ, UebertragungAn_Ort, UebertragungAn_Kanton, UebertragungAn_KesBehoerdeID, KesAufhebungsgrundCode, Bemerkung, Zustaendig_KesBehoerdeID, Creator, Created, Modifier, Modified, IsDeleted)
  SELECT 
    KesMassnahmeID                              = ALT.KesMassnahmeBSSID,
    FaLeistungID                                = ALT.FaLeistungID, 
    IsKS                                        = CASE WHEN ALT.KesMassnahmeTypCode = 2 THEN 1 ELSE 0 END, -- 1	Erwachsenenschutz, 2	Kindesschutz
    DocumentID_Errichtungsbeschluss             = ALT.KESBDocumentID, 
    DocumentID_Aufhebungsbeschluss              = NULL, 
    DatumVon                                    = ALT.ErrichtungVom, --COALESCE(ALT.AenderungVom, ALT.UebernahmeVom, ALT.ErrichtungVom),  -- Spalte Gültig Ab auf der Maske
    DatumBis                                    = COALESCE(ALT.Beistandswechsel, ALT.AufhebungVom), --COALESCE(ALT.Beistandswechsel, ALT.UebertragungVom, ALT.AufhebungVom, ALT.BerichtsgenehmigungVom),  -- Spalte Gültig bis auf der Maske
    KesAufgabenbereichCodes                     = ALT.KesAufgabenbereichCodes, 
    KesIndikationCodes                          = ALT.KesIndikationCodes, 
    Auftragstext                                = NULL, 
    KesElterlicheSorgeObhutCode_ElterlicheSorge = ALT.KesElterlicheSorgeObhutCode_ElterlicheSorge, 
    KesElterlicheSorgeObhutCode_Obhut           = ALT.KesElterlicheSorgeObhutCode_Obhut, 
    FuersorgerischeUnterbringung                = ALT.FuersorgerischeUnterbringung,
    Platzierung_BaInstitutionID                 = ALT.Platzierung_BaInstitutionID,
    KesGrundlageCode                            = ALT.KesGrundlageCode,
    UebernahmeVon_Datum                         = ALT.UebernahmeVom, 
    ...
    AenderungVom_Datum                          = ALT.AenderungVom, 
    KesAenderungsgrundCode                      = ALT.KesAenderungsgrundCode, 
    UebertragungAn_Datum                        = ALT.UebertragungVom, 
    UebertragungAn_OrtschaftCode                = ALT.OrtschaftCode, 
    UebertragungAn_PLZ                          = ALT.PLZ, 
    UebertragungAn_Ort                          = ALT.Ort, 
    UebertragungAn_Kanton                       = ALT.Kanton, 
    UebertragungAn_KesBehoerdeID                = NULL, 
    KesAufhebungsgrundCode                      = ALT.KesAufhebungsgrundCode, 
...
PRINT ('Info: ' + CONVERT(VARCHAR(MAX), @@ROWCOUNT) + ' Massnahme migriert');

SET IDENTITY_INSERT dbo.KesMassnahme OFF

-------------------------------------------------------------------------------
-- step 2: Mandatsführende Person
-------------------------------------------------------------------------------
INSERT INTO dbo.KesMandatsfuehrendePerson ( KesMassnahmeID ,
  ...
  SELECT 
    KesMassnahmeID               = ALT.KesMassnahmeBSSID,
    DatumMandatVon               = ALT.ErrichtungVom,
    DatumMandatBis               = NULL,
    DocumentID_Ernennungsurkunde = ALT.DocumentID_Urkunde,
    UserID                       = ALT.UserID,
...
PRINT ('Info: ' + CONVERT(VARCHAR(MAX), @@ROWCOUNT) + ' Mandatsführende Person migriert');
...
INSERT INTO dbo.KesMassnahme_KesArtikel (KesMassnahmeID, KesArtikelID, Creator, Created, Modifier, Modified)
  SELECT 
    KesMassnahmeID = KesMassnahmeBSSID,
    KesArtikelID   = KesArtikelID_MassnahmegebundeneGeschaefte,
    Creator        = Creator,
    Created        = Created,
    Modifier       = Modifier,
    Modified       = Modified
  FROM dbo.KesMassnahmeBSS WITH (READUNCOMMITTED)
  WHERE KesArtikelID_MassnahmegebundeneGeschaefte IS NOT NULL

PRINT ('Info: ' + CONVERT(VARCHAR(MAX), @@ROWCOUNT) + ' Artikel (Massnahmegebundene Geschaefte) migriert');


INSERT INTO dbo.KesMassnahme_KesArtikel (KesMassnahmeID, KesArtikelID, Creator, Created, Modifier, Modified)
  SELECT 
    KesMassnahmeID = KesMassnahmeBSSID,
    KesArtikelID   = KesArtikelID_NichtMassnahmegebundeneGeschaefte,
    Creator        = Creator,
    Created        = Created,
    Modifier       = Modifier,
    Modified       = Modified
  FROM dbo.KesMassnahmeBSS WITH (READUNCOMMITTED)
  WHERE KesArtikelID_NichtMassnahmegebundeneGeschaefte IS NOT NULL
    AND NOT EXISTS (SELECT TOP 1 1
                    FROM dbo.KesMassnahme_KesArtikel WITH (READUNCOMMITTED)
                    WHERE KesMassnahmeID = KesMassnahmeBSSID
                      AND KesArtikelID = KesArtikelID_NichtMassnahmegebundeneGeschaefte) --Sicherstellen, dass keine doppelten Daten existieren, falls KesArtikelID_MassnahmegebundeneGeschaefte = KesArtikelID_NichtMassnahmegebundeneGeschaefte

PRINT ('Info: ' + CONVERT(VARCHAR(MAX), @@ROWCOUNT) + ' Artikel (nicht Massnahmegebundene Geschaefte) migriert');


-------------------------------------------------------------------------------
-- step 4: BerichtsgenehmigungVom
-------------------------------------------------------------------------------
INSERT INTO dbo.KesMassnahmeBericht (KesMassnahmeID, KesMassnahmeBerichtsartCode, DatumVerfuegungKESB, Creator, Created, Modifier, Modified)
  SELECT																							   
    KesMassnahmeID              = KesMassnahmeBSSID,
    KesMassnahmeBerichtsartCode = 1, --1: Ordentlicher Bericht
    DatumVerfuegungKESB         = BerichtsgenehmigungVom,
    Creator                     = Creator,
    Created                     = Created,
    Modifier                    = Modifier,
    Modified                    = Modified
  FROM dbo.KesMassnahmeBSS WITH (READUNCOMMITTED)
  WHERE BerichtsgenehmigungVom IS NOT NULL
...
SET  @CURRENTDBNAME =  'KiSS_PlausEx_central';  
...
=================================================================================================
  Description
-------------------------------------------------------------------------------------------------
  SUMMARY: Use this script to fix default language of server-user "kiss" and "kiss_bm"
...
-------------------------------------------------------------------------------
IF (EXISTS (SELECT * FROM sys.server_principals WHERE name = N'kiss'))
BEGIN
  -- fix language
  ALTER LOGIN [kiss] WITH DEFAULT_LANGUAGE=[us_english];
  
  -- info
  PRINT ('Info: Set default language of server-user "kiss" to "us_english"');
END
ELSE
BEGIN
  -- info
  PRINT ('Warning: Could not fix default language of server-user "kiss" to "us_english", user not found!');
END;
GO
...
BEGIN
  ...
  SET @DocDBName = 'KiSS_PlausEx_central_DOC';
...
/*
************************************************************
  PROCESSED 139 FILES IN 00:00:09.7347301
************************************************************
*/
PRINT ('
************************************************************
