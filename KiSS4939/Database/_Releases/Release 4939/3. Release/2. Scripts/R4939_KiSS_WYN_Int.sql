...
************************************************************
  Settings:
    ...
    databaseName: [KiSS_WYN_Int]
    databaseName document: [KiSS_WYN_Int]
...
IF (DB_ID('KiSS_WYN_Int') IS NULL)
BEGIN
  RAISERROR('The Database ''KiSS_WYN_Int'' does not exist on the server!', 20, -1) WITH LOG;
END;
GO
...
BEGIN
  ...
  -- create login
  CREATE LOGIN [kiss_bm] WITH PASSWORD=N'textmarken88$', DEFAULT_DATABASE=[KiSS_WYN_Int], CHECK_EXPIRATION=OFF;
END
ELSE
BEGIN
  -- info
...
USE [msdb]
GO
/****** Object:  Job [KiSS_WYN_Int_Pendenzen]    Script Date: 02/11/2009 09:33:40 ******/
DECLARE @job_id varchar(100)

SELECT @job_id = job_id FROM msdb.dbo.sysjobs_view WHERE name = N'KiSS_WYN_Int_Pendenzen'

IF  @job_id IS NOT NULL
  EXEC msdb.dbo.sp_delete_job @job_id, @delete_unused_schedule=1
GO
 
/****** Object:  Job [KiSS_WYN_Int_Pendenzen]    Script Date: 02/11/2009 09:31:53 ******/
BEGIN TRANSACTION
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'KiSS_WYN_Int_Pendenzen', 
		@enabled= @enableJob, 
...
SET  @CURRENTDBNAME =  'KiSS_WYN_Int';  
...
/*
************************************************************
  PROCESSED 146 FILES IN 00:00:10.0314888
************************************************************
*/
PRINT ('
************************************************************
