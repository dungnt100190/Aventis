...
************************************************************
  Settings:
    ...
    searchFolder: [.]
    namespace extension: [WYN;Standard;~ZH;$Standard]
    databaseName: [KiSS_WYN_Prod]
    databaseName document: [KiSS_WYN_Prod]
...
IF (DB_ID('KiSS_WYN_Prod') IS NULL)
BEGIN
  RAISERROR('The Database ''KiSS_WYN_Prod'' does not exist on the server!', 20, -1) WITH LOG;
END;
GO
...
BEGIN
  ...
  -- create login
  CREATE LOGIN [kiss_bm] WITH PASSWORD=N'textmarken88$', DEFAULT_DATABASE=[KiSS_WYN_Prod], CHECK_EXPIRATION=OFF;
END
ELSE
BEGIN
  -- info
...
USE [msdb]
GO
/****** Object:  Job [KiSS_WYN_Prod_Pendenzen]    Script Date: 02/11/2009 09:33:40 ******/
DECLARE @job_id varchar(100)

SELECT @job_id = job_id FROM msdb.dbo.sysjobs_view WHERE name = N'KiSS_WYN_Prod_Pendenzen'

IF  @job_id IS NOT NULL
  EXEC msdb.dbo.sp_delete_job @job_id, @delete_unused_schedule=1
GO
 
/****** Object:  Job [KiSS_WYN_Prod_Pendenzen]    Script Date: 02/11/2009 09:31:53 ******/
BEGIN TRANSACTION
...
DECLARE @jobId BINARY(16),
        @enableJob BIT;
SET @enableJob = case when N';WYN;Standard;~ZH;$Standard;' LIKE '%;RESTORE;%' THEN 0 ELSE 1 END;
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'KiSS_WYN_Prod_Pendenzen', 
		@enabled= @enableJob, 
...
SET  @CURRENTDBNAME =  'KiSS_WYN_Prod';  
...
IF ((@DBWholeSize > 200 AND @DBWholeSize < 20480) OR ((';' + 'WYN;Standard;~ZH;$Standard' + ';') LIKE '%;PI;%'))
BEGIN
  -- do change size
  SET @ChangeSizes = 1;
...
/* -------- INFO: Start of file 'E:\dtvs0034\kiss4_agent1\_work\42\s\_Releases\Recurring\ChangeScripts\Rxxxx_Select_UserSession.sql' -------- */
PRINT ('=== start of file [E:\dtvs0034\kiss4_agent1\_work\42\s\_Releases\Recurring\ChangeScripts\Rxxxx_Select_UserSession.sql]: ' + CONVERT(VARCHAR, GETDATE(), 113) + ' ===')
GO
/*===============================================================================================
  $Revision: 3 $
=================================================================================================
...
GO
PRINT ('=== end of file [E:\dtvs0034\kiss4_agent1\_work\42\s\_Releases\Recurring\ChangeScripts\Rxxxx_Select_UserSession.sql]: ' + CONVERT(VARCHAR, GETDATE(), 113) + ' ===')
GO
/* -------- INFO: End of file 'E:\dtvs0034\kiss4_agent1\_work\42\s\_Releases\Recurring\ChangeScripts\Rxxxx_Select_UserSession.sql' -------- */
...
/*
************************************************************
  PROCESSED 146 FILES IN 00:00:07.3168978
************************************************************
*/
PRINT ('
************************************************************
