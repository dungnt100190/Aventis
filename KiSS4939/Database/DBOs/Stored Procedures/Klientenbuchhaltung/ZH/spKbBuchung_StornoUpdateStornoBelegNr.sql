...
SET ANSI_NULLS ON 
GO
EXECUTE dbo.spDropObject spKbBuchung_StornoUpdateStornoBelegNr
GO
/*===============================================================================================
  $Revision: 9 $
=================================================================================================
...
CREATE PROCEDURE dbo.spKbBuchung_StornoUpdateStornoBelegNr
(
	@KbBuchungID		int,
...
AS BEGIN
	...
	IF @Count > 0 BEGIN
		...
		SET		StornoBelegNr = @StornoBelegNr,
				StornoDatum = CURRENT_TIMESTAMP
		WHERE KbBuchungID = @KbBuchungID
	END
	ELSE BEGIN
		-- Es gibt keinen Eintrag für diese Buchung in der Storno-Tabelle => Fehler
		DECLARE @msg varchar(200)
		SET @msg = 'Der Netto-Beleg ' + CONVERT(varchar, @KbBuchungID) + ' existiert nicht in der KbBuchungStorno Tabelle, das ist aber Voraussetzung für das Abfüllen der Storno-Belegnummer ' + CONVERT(varchar, @StornoBelegNr)
