...
SELECT 
  ...
  Stichtag            = CASE DOS.Stichtag WHEN 1 THEN 'ja' ELSE 'nein' END,
       [Dossier-Nr.]       = DOS.BFSDossierID, --DOS.FaLeistungID,
       FallNr              = LEI.FaFallID,
       LeistungNr          = DOS.FaLeistungID,
...
DECLARE C CURSOR FOR
  ...
  WHERE FRA.BFSKatalogVersionID = dbo.fnBFSGetKatalogVersion(@Erhebungsjahr) 
                AND ',' + FRA.BFSLeistungsfilterCodes + ',' like '%,' + CONVERT(varchar,case @BFSLeistungsartCode
                                                                                  when 1 then 1
                                                                                  when 2 then 1
                                                                                  when 23 then 5
                                                                                  when 25 then 4
...
if @BFSLeistungsartCode in (1,2) begin
  -- WSH: Datum erste Auszahlung für alle UE/HH-Mitglieder Wert des Antragstellers duplizieren 
  set @ColName = CASE @ExcelExport WHEN 1 THEN 'V15.06 Datum der ersten Auszahlung' ELSE 'V15.06' END
  set @sql = 'update T
              set    [' + @ColName + '] = T2.[' + @ColName + ']
              from   #tmp T
                     inner join #tmp T2 on T2.BFSDossierID$ = T.BFSDossierID$ and
                                                T2.BFSPersonCode$ = 1
              where T.BFSPersonCode$ in (2,3)'
  execute (@sql)

  -- WSH: Datum letzte Auszahlung für alle UE/HH-Mitglieder Wert des Antragstellers duplizieren 
  set @ColName = CASE @ExcelExport WHEN 1 THEN 'V16.02 Datum der letzten Auszahlung' ELSE 'V16.02' END
  set @sql = 'update T
              set    [' + @ColName + '] = T2.[' + @ColName + ']
              from   #tmp T
                     inner join #tmp T2 on T2.BFSDossierID$ = T.BFSDossierID$ and
