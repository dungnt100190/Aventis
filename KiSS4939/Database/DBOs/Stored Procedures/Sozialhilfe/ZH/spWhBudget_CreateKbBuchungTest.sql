...
SET ANSI_NULLS ON 
GO
EXECUTE dbo.spDropObject spWhBudget_CreateKbBuchungTest
GO

CREATE PROCEDURE [dbo].[spWhBudget_CreateKbBuchungTest]
(
  @BgBudgetID          int,
  @UserID              int,
...
PRINT 'b'
  ...
  WHERE  BgBudgetID = @BgBudgetID AND BgPositionID = IsNull(@BgPositionID_IN,BgPositionID) AND KbBuchungTypCode IN (1/*Budget*/) AND KbBuchungStatusCode NOT IN (5,8,9) --falls Belege in diesen Sati existieren, können neue erzeugt werden 

  SELECT @COUNT = @COUNT + COUNT(*)
  FROM   KbBuchungBrutto             KBB
    INNER JOIN KbBuchungBruttoPerson KBP ON KBP.KbBuchungBruttoID = KBB.KbBuchungBruttoID
  WHERE  BgBudgetID = @BgBudgetID AND BgPositionID = IsNull(@BgPositionID_IN,BgPositionID) AND KbBuchungTypCode IN (1/*Budget*/) AND KbBuchungStatusCode NOT IN (5,8,9)
  ...
  WHERE BPO.BgBudgetID = @BgBudgetID AND BPO.BgKategorieCode IN (1, 2, 100) -- Einnahmen, Ausgaben, Einzelzahlungen
--    AND (BPO.Betrag - BPO.Reduktion) > 0
  ...
  WHERE BPO.BgBudgetID = @BgBudgetID
    AND (BPO.BgKategorieCode = 101 OR (BPO.BgKategorieCode IN (2, 100) AND BgSpezkontoID IS NULL)) -- Einnahmen, Ausgaben, Einzelzahlungen
    AND (BPO.Betrag - BPO.Reduktion) > 0
  ...
  SELECT @COUNT = COUNT(*)
  FROM   #vwBgPosition           BPO
    LEFT JOIN BgAuszahlungPerson BAP ON BPO.BgPositionID = BAP.BgPositionID
  WHERE  BgBudgetID = @BgBudgetID AND BAP.ReferenzNummer IS NOT NULL AND dbo.[fnCheckReferenznummer](BAP.ReferenzNummer) = 0

  IF (@COUNT > 0) BEGIN
    RAISERROR ( 'Nicht alle Refernznummern sind gültig!', 18, 1)
    RETURN -1
  END
  ...
  SELECT @COUNT = COUNT(*)
  FROM   #vwBgPosition
  WHERE  BgBudgetID = @BgBudgetID AND BgKategorieCode = 1 /*Einnahme*/ AND VerwaltungSD = 1 AND Betrag > 0 AND BaInstitutionID IS NULL AND DebitorBaPersonID IS NULL
  ...
  WHERE BPO.BgBudgetID = @BgBudgetID AND BKA.Quoting = 0 AND BPO.BaPersonID IS NULL
    AND (BPO.Betrag - BPO.Reduktion) > 0
...
PRINT 'e'
  ...
  IF (@COUNT > 0) BEGIN
    SET @msg = 'Nicht alle Personen dieses Finanzplans haben eine Wohn-/Meldeadresse. Die WMA ist Voraussetzung, um Auszahlungen zu machen'
    RAISERROR (@msg, 18, 1)
    RETURN -1
  END
...
*/
  ...
  FROM   BaBank
  WHERE  ClearingNr = '9000' AND LandCode = 8100
  ...
  WHILE 1=1 BEGIN
    ...
    IF( @KbAuszahlungsArtCode <> 103 ) BEGIN
      ...
      INSERT INTO @KbBuchung (KbPeriodeID, KbBuchungTypCode, BelegDatum, ValutaDatum, Betrag, [Text], KbBuchungStatusCode, BaZahlungswegID, SollKtoNr, HabenKtoNr,
                              KbAuszahlungsArtCode, PCKontoNr, BaBankID, BankKontoNr, IBAN, ReferenzNummer, Zahlungsgrund, BeguenstigtName, BeguenstigtName2, BeguenstigtStrasse, BeguenstigtPLZ, BeguenstigtOrt,
        ...
        SELECT @KbPeriodeID, 1/*Budget*/, GetDate(), @ValutaDatum, @SumBetrag,
             'an ' + CASE WHEN ZAL.BaZahlungswegID IS NOT NULL THEN CAST(CASE WHEN ZAL.BaPersonID IS NOT NULL THEN PRS.NameVorname WHEN ZAL.BaInstitutionID IS NOT NULL THEN INS.Name END AS varchar) + '; am ' + CONVERT(varchar,@ValutaDatum,104) WHEN @KbAuszahlungsArtCode = 103 THEN 'Barauszahlung' ELSE '' END,
             @KbBuchungStatusCode, @BaZahlungswegID, NULL, @KontoNrKreditor,
             @KbAuszahlungsArtCode, ZAL.PostKontoNummer, ZAL.BaBankID, ZAL.BankKontoNummer, ZAL.IBANNummer, @ReferenzNummer, @Zahlungsgrund,
               CASE WHEN ZAL.BaPersonID IS NOT NULL THEN PRS.NameVorname           ELSE INS.Name END,
               NULL,
               CASE WHEN ZAL.BaPersonID IS NOT NULL THEN PRS.WohnsitzStrasseHausNr ELSE INS.StrasseHausNr   END,
               CASE WHEN ZAL.BaPersonID IS NOT NULL THEN PRS.WohnsitzPLZ           ELSE INS.PLZ             END,
               CASE WHEN ZAL.BaPersonID IS NOT NULL THEN PRS.WohnsitzOrt           ELSE INS.Ort             END,
             @BgBudgetID,
               IsNull( XLA.Value1, XLE.Value1 ), ModulID = 3 /*W*/, @Kostenstelle,
             BNK.Name, BNK.Strasse, BNK.PLZ, BNK.Ort, BNK.ClearingNr, @UserID, GetDate(), @FaLeistungID, @MitteilungZeile1, @MitteilungZeile2, @MitteilungZeile3, @MitteilungZeile4
        FROM BaZahlungsweg         ZAL
          ...
          LEFT  JOIN XLOVCode      XLE ON XLE.LOVName = 'BgEinzahlungsschein' AND XLE.Code = ZAL.EinzahlungsscheinCode
          LEFT  JOIN XLOVCode      XLA ON XLA.LOVName = 'KbAuszahlungsArt'    AND XLA.Code = @KbAuszahlungsArtCode
          LEFT  JOIN BaBank        BNK ON BNK.BaBankID = CASE WHEN ZAL.BaBankID IS NULL AND ZAL.PostKontoNummer IS NOT NULL THEN @BaBankID_Post ELSE ZAL.BaBankID END
        WHERE ZAL.BaZahlungswegID = @BaZahlungswegID
        GROUP BY ZAL.BaZahlungswegID, ZAL.PostKontoNummer, ZAL.BaBankID, ZAL.BankKontoNummer, ZAL.IBANNummer, ZAL.BaPersonID, ZAL.BaInstitutionID, ZAL.EinzahlungsscheinCode ,PRS.NameVorname, PRS.WohnsitzStrasseHausNr, PRS.WohnsitzPLZ, PRS.WohnsitzOrt, INS.Name, INS.StrasseHausNr, INS.PLZ, INS.Ort, XLA.Value1, XLE.Value1, BNK.Name, BNK.Strasse, BNK.PLZ, BNK.Ort, BNK.ClearingNr
    END
    ELSE BEGIN
      -- Barauszahlung
  ...
  --ZuErzeugendeVerbuchteEinnahmen, nur Vorschau - bei echten Belegen muss der ganze Vorgang wegen dem Op-Ausgleich in der Transaktion geschehen
/*
IF @PreviewMode = 1 BEGIN

  DECLARE cNettoBeleg CURSOR FOR
    SELECT ValutaDatum, FLOOR((SUM(EIN.Betrag)) * 20.0 + 0.5) / 20.0,
           BPO.BaInstitutionID, DebitorBaPersonID, BKA.KontoNr
    FROM @ZuErzeugendeVerbuchteEinnahmen EIN
      LEFT JOIN BgPosition               BPO ON BPO.BgPositionID     = EIN.BgPositionID
      LEFT JOIN BgSpezkonto              SPZ ON SPZ.BgSpezkontoID    = BPO.BgSpezkontoID
      LEFT JOIN BgPositionsart           BPA ON BPA.BgPositionsartID = SPZ.BgPositionsartID
      LEFT JOIN BgKostenart              BKA ON BKA.BgKostenartID    = BPA.BgKostenartID
    GROUP BY ValutaDatum, BPO.BaInstitutionID, DebitorBaPersonID, BKA.KontoNr

  OPEN cNettoBeleg
  WHILE 1=1 BEGIN
    FETCH NEXT FROM cNettoBeleg INTO @ValutaDatum, @SumBetrag, @Schuldner_BaInstitutionID, @Schuldner_BaPersonID, @KontoNr
    IF @@FETCH_STATUS < 0 BREAK

    -- Buchungskopf 
      INSERT INTO @KbBuchung (KbPeriodeID, KbBuchungTypCode, BelegDatum, ValutaDatum, Betrag, [Text], KbBuchungStatusCode, BaZahlungswegID,
                              KbAuszahlungsArtCode, ReferenzNummer, BgBudgetID, /*KbKostenstelleID,*/ SollKtoNr, ErstelltUserID, ErstelltDatum)
      VALUES( @KbPeriodeID, 1/*Budget*/, GetDate(), @ValutaDatum, @SumBetrag, 'Forderung aus Verrechnung', 6/*ausgeglichen*/, NULL,
             NULL, NULL, @BgBudgetID, /*@KbKostenstelleID,*/ @KontoNrDebitor, @UserID, GetDate() )

      SET @KbBuchungID = SCOPE_IDENTITY()

      -- Detailpositionen
      INSERT INTO @KbBuchungKostenart (KbBuchungID, BgPositionID, BgKostenartID, BaPersonID, Buchungstext, Betrag, PositionImBeleg, Hauptvorgang, Teilvorgang, Belegart, KontoNr, VerwPeriodeVon, VerwPeriodeBis)--, Kostenstelle)
        SELECT @KbBuchungID, EIN.BgPositionID, EIN.BgKostenartID, EIN.BaPersonID, EIN.Text, EIN.Betrag, NULL,
               BKA.HauptvorgangAbtretung,
               BKA.TeilvorgangAbtretung,
               BKA.Belegart, BKA.KontoNr, BPO.VerwPeriodeVon, BPO.VerwPeriodeBis--, @Kostenstelle
        FROM   @ZuErzeugendeVerbuchteEinnahmen EIN
          INNER JOIN BgKostenart    BKA ON BKA.BgKostenartID    = EIN.BgKostenartID
          LEFT  JOIN BgPosition     BPO ON BPO.BgPositionID     = EIN.BgPositionID
        WHERE IsNull(BPO.BaInstitutionID,-1) = IsNull(@Schuldner_BaInstitutionID,-1) AND IsNull(BPO.DebitorBaPersonID,-1) = IsNull(@Schuldner_BaPersonID,-1)
...
/*
  ...
  */
  ...
  INSERT INTO @KbBuchungBrutto (KbPeriodeID, BgKostenartID, KbBuchungTypCode, BelegDatum, ValutaDatum, ErfassungsDatum, Betrag, [Text], KbBuchungStatusCode, UserID, BgBudgetID, Kostenstelle, BetragEffektiv, Hauptvorgang, Teilvorgang, Belegart, BgPositionID)
    SELECT @KbPeriodeID, BKA.BgKostenartID, 1/*Budget*/, GetDate(), TMP.Termin, GetDate(), FLOOR((-SUM(BetragBrutto)) * 20.0 + 0.5) / 20.0, BKA.Name, TMP.KbBuchungStatusCode, @UserID, @BgBudgetID, @Kostenstelle, $0.0,
      CASE TMP.VerwaltungSD WHEN 1 THEN BKA.HauptvorgangAbtretung ELSE BKA.Hauptvorgang END,
      CASE TMP.VerwaltungSD WHEN 1 THEN BKA.TeilvorgangAbtretung  ELSE BKA.Teilvorgang  END,
      BKA.Belegart,
      CASE Einnahme WHEN 1 THEN TMP.BgPositionID END
    FROM #BruttoAuszahlungen TMP
      INNER JOIN BgKostenart BKA ON BKA.BgKostenartID = TMP.BgKostenartID
    GROUP BY BKA.BgKostenartID, Termin, BKA.Name, KbBuchungStatusCode, VerwaltungSD, BKA.Hauptvorgang, BKA.Teilvorgang, BKA.HauptvorgangAbtretung, BKA.TeilvorgangAbtretung, BKA.Belegart, CASE Einnahme WHEN 1 THEN TMP.BgPositionID END
  ...
  INSERT INTO @KbBuchungBruttoPerson (KbBuchungBruttoID, BgPositionID, BaPersonID, Schuldner_BaInstitutionID, Schuldner_BaPersonID, Buchungstext, Betrag, VerwPeriodeVon, VerwPeriodeBis)
    SELECT KBN.KbBuchungBruttoID, AUZ.BgPositionID, AUZ.BaPersonID_Teil, Schuldner_BaInstitutionID, Schuldner_BaPersonID, AUZ.Name, -BetragBrutto,
      CASE BKA.BgSplittingArtCode WHEN 3 THEN @ValutaDatum ELSE IsNull(AUZ.VerwPeriodeVon, @FirstDayInMonth) END,
      CASE BKA.BgSplittingArtCode WHEN 3 THEN NULL         ELSE IsNull(AUZ.VerwPeriodeBis, @LastDayInMonth)  END
    FROM   #BruttoAuszahlungen    AUZ
      ...
      INNER JOIN @KbBuchungBrutto KBN ON KBN.BgKostenartID = AUZ.BgKostenartID AND AUZ.Termin = KBN.ValutaDatum
                                           ...
                                           AND AUZ.BgPositionID = CASE Einnahme WHEN 1 THEN KBN.BgPositionID ELSE AUZ.BgPositionID END
...
delete from KbBuchungKostenart where KbBuchungID IN (SELECT KbBuchungID FROM KbBuchung where BgBudgetID = @BgBudgetID)
...
/*
      --ZuErzeugendeVerbuchteEinnahmen
      IF @PreviewMode = 0 BEGIN
        DECLARE @KbBuchungID_Gegenbuchung int
        DECLARE cNettoBelegVerbuchteEinnahmen CURSOR FOR
          SELECT ValutaDatum, FLOOR((SUM(EIN.Betrag)) * 20.0 + 0.5) / 20.0,
                 BPO.BaInstitutionID, DebitorBaPersonID, BKA.KontoNr
          FROM @ZuErzeugendeVerbuchteEinnahmen EIN
            LEFT JOIN BgPosition               BPO ON BPO.BgPositionID     = EIN.BgPositionID
        ...
        OPEN cNettoBelegVerbuchteEinnahmen
        WHILE 1=1 BEGIN
          FETCH NEXT FROM cNettoBelegVerbuchteEinnahmen INTO @ValutaDatum, @SumBetrag, @Schuldner_BaInstitutionID, @Schuldner_BaPersonID, @KontoNr
          IF @@FETCH_STATUS < 0 BREAK

          -- Buchungskopf 
            INSERT INTO KbBuchung (KbPeriodeID, KbBuchungTypCode, BelegDatum, ValutaDatum, Betrag, [Text], KbBuchungStatusCode, BaZahlungswegID,
                                    KbAuszahlungsArtCode, ReferenzNummer, BgBudgetID, /*KbKostenstelleID,*/ SollKtoNr, ErstelltUserID, ErstelltDatum)
            VALUES( @KbPeriodeID, 1/*Budget*/, GetDate(), @ValutaDatum, @SumBetrag, 'Forderung', 6/*ausgeglichen*/, NULL,
                   NULL, NULL, @BgBudgetID, /*@KbKostenstelleID,*/ @KontoNrDebitor, @UserID, GetDate() )

            SET @KbBuchungID = SCOPE_IDENTITY()

            -- Detailpositionen
            INSERT INTO KbBuchungKostenart (KbBuchungID, BgPositionID, BgKostenartID, BaPersonID, Buchungstext, Betrag, PositionImBeleg, Hauptvorgang, Teilvorgang, Belegart, KontoNr, VerwPeriodeVon, VerwPeriodeBis)--, KbKostenstelleID)
              SELECT @KbBuchungID, EIN.BgPositionID, EIN.BgKostenartID, EIN.BaPersonID, EIN.Text, EIN.Betrag, NULL,
                     BKA.HauptvorgangAbtretung, BKA.TeilvorgangAbtretung, BKA.Belegart,
                     BKA.KontoNr, BPO.VerwPeriodeVon, BPO.VerwPeriodeBis--, @KbKostenstelleID
              FROM   @ZuErzeugendeVerbuchteEinnahmen EIN
                INNER JOIN BgKostenart               BKA ON BKA.BgKostenartID = EIN.BgKostenartID
                LEFT  JOIN BgPosition                BPO ON BPO.BgPositionID  = EIN.BgPositionID
              WHERE IsNull(BPO.BaInstitutionID,-1) = IsNull(@Schuldner_BaInstitutionID,-1) AND IsNull(BPO.DebitorBaPersonID,-1) = IsNull(@Schuldner_BaPersonID,-1)
            ...
            -- Gegenbuchung
            INSERT INTO KbBuchung (KbPeriodeID, KbBuchungTypCode, BelegDatum, ValutaDatum, Betrag, [Text], KbBuchungStatusCode, BaZahlungswegID,
                                    KbAuszahlungsArtCode, ReferenzNummer, BgBudgetID, /*KbKostenstelleID,*/ HabenKtoNr, SollKtoNr, ErstelltUserID, ErstelltDatum)
            VALUES( @KbPeriodeID, 4/*Ausgleich*/, GetDate(), @ValutaDatum, @SumBetrag, 'Ausgleich', 6/*ausgeglichen*/, NULL,
                   NULL, NULL, @BgBudgetID, /*@KbKostenstelleID,*/ @KontoNrDebitor, @KontoNr, @UserID, GetDate() )

            SET @KbBuchungID_Gegenbuchung = SCOPE_IDENTITY()

            --OP-Ausgleich
            INSERT INTO KbOpAusgleich (OpBuchungID, AusgleichBuchungID, Betrag)
            VALUES (@KbBuchungID, @KbBuchungID_Gegenbuchung, @SumBetrag)
        END
        CLOSE cNettoBelegVerbuchteEinnahmen
        DEALLOCATE cNettoBelegVerbuchteEinnahmen
      END
*/
      ...
      OPEN cKopf
      WHILE 1=1 BEGIN
        FETCH NEXT FROM cKopf INTO @KbBuchungBruttoID_tmp
        IF @@FETCH_STATUS <> 0 BREAK
        ...
        INSERT INTO KbBuchungBrutto ([KbPeriodeID], [BgKostenartID], [KbBuchungTypCode], [Code], [BelegNr], [BelegDatum], [ValutaDatum], [ErfassungsDatum], [TransferDatum], [Zahlungsfrist], [Betrag],
                                     ...
                                     [Kostenstelle], [Hauptvorgang], [Teilvorgang], [Belegart])
          SELECT [KbPeriodeID], [BgKostenartID], [KbBuchungTypCode], [Code], [BelegNr], [BelegDatum], [ValutaDatum], [ErfassungsDatum], [TransferDatum], [Zahlungsfrist], [Betrag],
                 [BetragEffektiv], [DatumEffektiv], [Text], [KbBuchungStatusCode], [UserID], [StorniertKbBuchungBruttoID], [BgBudgetID], [PscdFehlermeldung], [ModulID],
                 [Kostenstelle], [Hauptvorgang], [Teilvorgang], [Belegart]
          FROM @KbBuchungBrutto
...
/*
  DECLARE @KbBuchungBrutto TABLE(
	[KbPeriodeID], [BgKostenartID],	[KbBuchungTypCode],	[Code],	[BelegNr], [BelegDatum], [ValutaDatum], [ErfassungsDatum], [TransferDatum], [Zahlungsfrist], [Betrag],
    [BetragEffektiv], [DatumEffektiv], [Text], [KbBuchungStatusCode], [UserID], [StorniertKbBuchungBruttoID], [BgBudgetID], [PscdFehlermeldung], [ModulID],
    [Kostenstelle], [Hauptvorgang], [Teilvorgang], [Belegart]
  )

  DECLARE @KbBuchungBruttoPerson TABLE(
	[KbBuchungBruttoID], [BgPositionID], [BaPersonID], [Schuldner_BaInstitutionID], [Schuldner_BaPersonID],	[Buchungstext], [Betrag], [VerwPeriodeVon], [VerwPeriodeBis], [PositionImBeleg])
*/
...
/*
  ...
  SELECT * FROM @KbBuchungBrutto
  SELECT * FROM @KbBuchungBruttoPerson
*/
PRINT 'u'
