...
SET ANSI_NULLS ON 
GO
EXECUTE dbo.spDropObject spWhKontoauszug_Test_wbm
GO
/*===============================================================================================
...
CREATE PROCEDURE dbo.spWhKontoauszug_Test_wbm
 (@FallBaPersonID int,
  @BaPersonID int,
...
IF @debug = 1 print '2: ' + convert(varchar,DATEDIFF(ms, GETDATE(), @StartTime)) + ' ms'
  ...
  INSERT @tmp
  SELECT 
	distinct BuchungID           = KBB.KbBuchungBruttoID,
	...
	KbBuchungStatusCode = CASE WHEN KBBviaSTO.KbBuchungBruttoID IS NOT NULL		
    ...
    StornoCode          = CASE 
                          WHEN KBB.KbBuchungBruttoID = KBBviaNEU.KbBuchungBruttoID THEN 2
                          WHEN KBB.KbBuchungBruttoID = KBBviaSTO.KbBuchungBruttoID THEN 1
                          ELSE 0
                          END
  FROM dbo.FaFall                            FAL WITH (READUNCOMMITTED) 
	...
	INNER JOIN dbo.KbBuchungBrutto           KBBviaBudget WITH (READUNCOMMITTED) ON KBBviaBudget.BgBudgetID        = BBG.BgBudgetID
	LEFT JOIN dbo.KbBuchungBrutto            KBBviaSTO WITH (READUNCOMMITTED) ON KBBviaSTO.StorniertKbBuchungBruttoID = KBBviaBudget.KbBuchungBruttoID
	LEFT JOIN dbo.KbBuchungBrutto            KBBviaNEU WITH (READUNCOMMITTED) ON KBBviaNEU.NeubuchungvonkbbuchungbruttoID = KBBviaBudget.KbBuchungBruttoID	
	INNER JOIN dbo.KbBuchungBrutto           KBB WITH (READUNCOMMITTED) ON KBB.KbBuchungBruttoID = KBBviaBudget.KbBuchungBruttoID 
																		   OR KBB.KbBuchungBruttoID = KBBviaSTO.KbBuchungBruttoID 
																	       OR KBB.KbBuchungBruttoID = KBBviaNEU.KbBuchungBruttoID
...
IF @debug = 1 print '8: ' + convert(varchar,DATEDIFF(ms, GETDATE(), @StartTime)) + ' ms'
...
IF @debug = 1 print '9: ' + convert(varchar,DATEDIFF(ms, GETDATE(), @StartTime)) + ' ms'
...
IF @debug = 1 print '10: ' + convert(varchar,DATEDIFF(ms, GETDATE(), @StartTime)) + ' ms'
...
IF @debug = 1 print '11: ' + convert(varchar,DATEDIFF(ms, GETDATE(), @StartTime)) + ' ms'
