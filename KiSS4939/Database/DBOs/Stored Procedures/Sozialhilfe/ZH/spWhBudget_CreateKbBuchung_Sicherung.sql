...
SET ANSI_NULLS ON 
GO
EXECUTE dbo.spDropObject spWhBudget_CreateKbBuchung_Sicherung
GO

CREATE PROCEDURE [dbo].[spWhBudget_CreateKbBuchung_Sicherung]
(
  @BgBudgetID          int,
  @UserID              int,
...
--      INNER JOIN BgAuszahlungPerson  STZ ON STZ.BgPositionID   = BPO.BgPositionID
    ...
    WHERE BPO.BgBudgetID = @BgBudgetID AND BPO.BgKategorieCode IN (101) -- Einzelzahlung
      ...
      AND dbo.fnBgSpezkonto(BPO.BgSpezkontoID, CASE WHEN BgSpezkontoTypCode IN (1,2) THEN 3 ELSE 4 END, @BgBudgetID, @BgPositionID_IN) <= CASE WHEN @BgPositionID_IN IS NOT NULL THEN BPO.Betrag ELSE $0.00 END
*/
...
--bgfinanzplanid, fafallid
  ...
  INSERT INTO #Buchungen (BgPositionID, BaPersonID, BaPersonID_Teil, BetragBrutto, BetragNetto, Kostenstelle, BgPositionsartID, [Name], BgKostenartID, Einnahme, BgKategorieCode, BgSpezkontoID, VerwaltungSD, KbBuchungStatusCode, BgSplittingArtCode)
    ...
    FROM #vwBgPosition               BPO
      ...
      LEFT OUTER JOIN dbo.BgPositionsart      BPA WITH (READUNCOMMITTED) ON  BPA.BgPositionsartID = SPZ.BgPositionsartID AND BPA.BgKategorieCode = BPO.BgKategorieCode
      LEFT OUTER JOIN dbo.BgKostenart         BKA WITH (READUNCOMMITTED) ON  BKA.BgKostenartID    = IsNull(BPA.BgKostenartID,SPZ.BgKostenartID)
    WHERE BPO.BgBudgetID = @BgBudgetID
        ...
        IF @SumZulage > @Limit BEGIN
          INSERT INTO @Abzuege (BgPositionID, BaPersonID, BetragNetto, BetragBrutto, BgKategorieCode)
            SELECT BPO.BgPositionID, BPO.BaPersonID,
              ...
              BPO.Betrag - (BPO.Betrag * @Limit / @SumZulage), BPO.BgKategorieCode
            FROM #vwBgPosition  BPO
              INNER JOIN dbo.BgPositionsart BPA WITH (READUNCOMMITTED) ON BPA.BgPositionsartID = BPO.BgPositionsartID
            WHERE BPO.BgBudgetID = @BgBudgetID AND BPO.BgKategorieCode = 2 -- Ausgaben
              AND BPA.BgGruppeCode BETWEEN 39000 AND 39999 /* EFB, IZU, MIZ */
...
*/
  SELECT AuszahlBetrag = SUM(IsNull(CAST(BUC.NettoBetragProTermin AS money),BUC.BetragNetto)), BUC.BgPositionID, BUC.BaPersonID_Teil, BUC.Kostenstelle, BUC.BgPositionsartID, BUC.Name, BUC.BgKostenartID, BUC.Einnahme, Datum = IsNull(BAT.Datum,POS.FaelligAm), BUC.TerminFaktor, BAP.KbAuszahlungsArtCode, BAP.BaZahlungswegID, BAP.ReferenzNummer, Zahlungsgrund = NULL,/*BAP.Zahlungsgrund,*/ BUC.VerwaltungSD, BUC.Dritte, BUC.BgKategorieCode, BUC.KbBuchungStatusCode, MitteilungZeile1 = CASE BAP.KbAuszahlungsArtCode WHEN 103 /*Bar*/ THEN BAP.MitteilungZeile1 END, BAP.MitteilungZeile2, BAP.MitteilungZeile3, BAP.MitteilungZeile4, Abgetreten = CASE WHEN BUC.BgKategorieCode = 3 OR BUC.VerwaltungSD = 1 THEN 1 ELSE 0 END
    ...
    GROUP BY BUC.BgPositionID, BUC.BaPersonID_Teil, BUC.Kostenstelle, BUC.BgPositionsartID, BUC.Name, BUC.BgKostenartID, BUC.Einnahme, BAP.BaZahlungswegID, IsNull(BAT.Datum,POS.FaelligAm), BAP.KbAuszahlungsArtCode, BUC.TerminFaktor, BAP.ReferenzNummer, /*BAP.Zahlungsgrund,*/ BUC.VerwaltungSD, BUC.Dritte, BUC.BgKategorieCode, BUC.KbBuchungStatusCode, BUC.AuszahlGruppeID, CASE BAP.KbAuszahlungsArtCode WHEN 103 /*Bar*/ THEN BAP.MitteilungZeile1 END,BAP.MitteilungZeile2, BAP.MitteilungZeile3, BAP.MitteilungZeile4, CASE WHEN BUC.BgKategorieCode = 3 OR BUC.VerwaltungSD = 1 THEN 1 ELSE 0 END
    ORDER BY BUC.BaPersonID_Teil, BUC.AuszahlGruppeID
...
*/
  ...
  INSERT INTO @ZahlungswegeCheck
    ...
    FROM #Buchungen            BUC
      ...
      INNER JOIN dbo.BaZahlungsweg ZAL WITH (READUNCOMMITTED) ON ZAL.BaPersonID   = POS.DebitorBaPersonID OR ZAL.BaInstitutionID = POS.BaInstitutionID
    WHERE ZAL.BaZahlungswegID IS NOT NULL
  ...
  INSERT INTO @WMACheck
    ...
    FROM #NettoAuszahlungen    AUZ
      ...
      LEFT OUTER JOIN dbo.BaAdresse     ADR WITH (READUNCOMMITTED) ON ADR.BaPersonID = ZAL.BaPersonID OR ADR.BaInstitutionID = ZAL.BaInstitutionID
  ...
  INSERT INTO @WMACheck
    ...
    FROM #Buchungen            BUC
      INNER JOIN dbo.BgPosition    POS WITH (READUNCOMMITTED) ON POS.BgPositionID = BUC.BgPositionID
      LEFT OUTER JOIN dbo.BaAdresse    ADR WITH (READUNCOMMITTED) ON ADR.BaPersonID   = POS.DebitorBaPersonID OR ADR.BaInstitutionID = POS.BaInstitutionID
...
/*
    SELECT '#NettoAuszahlungen', Einnahme, Datum, FLOOR((SUM(AuszahlBetrag)) * 20.0 + 0.5) / 20.0,
...
*/
      ...
      INSERT INTO @KbBuchungKostenart (KbBuchungID, BgPositionID, BgKostenartID, BaPersonID, Buchungstext, Betrag, PositionImBeleg, Hauptvorgang, Teilvorgang, Belegart, KontoNr, VerwPeriodeVon, VerwPeriodeBis)
        SELECT @KbBuchungID, AUZ.BgPositionID, AUZ.BgKostenartID, AUZ.BaPersonID_Teil, AUZ.Name, AuszahlBetrag, NULL,
               CASE AUZ.Abgetreten WHEN 1 THEN BKA.HauptvorgangAbtretung ELSE BKA.Hauptvorgang END,
               CASE AUZ.Abgetreten WHEN 1 THEN BKA.TeilvorgangAbtretung  ELSE BKA.Teilvorgang  END,
               BKA.Belegart,
               BKA.KontoNr,
...
*/
  ...
  IF @KissStandard = 0 BEGIN
    ...
    IF @KbBuchungIDGBL IS NULL BEGIN
      ...
      IF @KbBuchungIDGBL IS NULL AND @Count > 0 BEGIN
        INSERT INTO @KbBuchung (KbPeriodeID, KbBuchungTypCode, BelegDatum, ValutaDatum, Betrag, [Text], KbBuchungStatusCode, BaZahlungswegID,
                                KbAuszahlungsArtCode, ReferenzNummer, BgBudgetID, PscdZahlwegArt, ModulID, Kostenstelle, SollKtoNr, ErstelltUserID, ErstelltDatum, Schuldner_BaPersonID, Schuldner_BaInstitutionID, FaLeistungID, Einnahme)
        VALUES(@KbPeriodeID, 1/*Budget*/, GetDate(), @FirstDayInMonth, 0.00, 'Dummy 0er-Beleg', @KbBuchungStatusCode, NULL,
               NULL, NULL, @BgBudgetID, NULL, 3, @Kostenstelle, @KontoNrDebitor, @UserID, GetDate(), NULL, NULL, @FaLeistungID, 1)
    ...
    IF @KbBuchungIDGBL IS NOT NULL BEGIN
      ...
      INSERT INTO @KbBuchungKostenart (KbBuchungID, BgPositionID, BgKostenartID, BaPersonID, Buchungstext, Betrag, PositionImBeleg, Hauptvorgang, Teilvorgang, Belegart, KontoNr, VerwPeriodeVon, VerwPeriodeBis)
        SELECT @KbBuchungIDGBL, BUC.BgPositionID, BUC.BgKostenartID, BUC.BaPersonID_Teil, BUC.Name, $0.00, NULL,
               CASE WHEN BUC.VerwaltungSD = 1 OR BUC.BgKategorieCode = 3 THEN BKA.HauptvorgangAbtretung ELSE BKA.Hauptvorgang END,
               CASE WHEN BUC.VerwaltungSD = 1 OR BUC.BgKategorieCode = 3 THEN BKA.TeilvorgangAbtretung  ELSE BKA.Teilvorgang  END,
               BKA.Belegart,
               BKA.KontoNr,
               VerwPeriodeVon = CASE BKA.BgSplittingArtCode WHEN 3 /*Valuta*/ THEN @ValutaDatum WHEN 4 /*Entscheid*/ THEN BPO.VerwPeriodeVon ELSE IsNull(BPO.VerwPeriodeVon, @FirstDayInMonth) END,
               VerwPeriodeBis = CASE BKA.BgSplittingArtCode WHEN 3 /*Valuta*/ THEN @ValutaDatum WHEN 4 /*Entscheid*/ THEN BPO.VerwPeriodeVon ELSE IsNull(BPO.VerwPeriodeBis, @LastDayInMonth)  END
...
*/
  ...
  SELECT BetragBrutto = SUM(IsNull(CAST(BruttoBetragProTermin AS money),BetragBrutto)), BUC.BgPositionID, BUC.BaPersonID_Teil, BUC.Kostenstelle, BUC.BgPositionsartID, BUC.Name, BUC.BgKostenartID, BUC.Einnahme, Termin = COALESCE(BAT.Datum, POS.FaelligAm, @FirstDayInMonth), Zahlungsgrund = NULL, /*BAP.Zahlungsgrund, */BUC.VerwaltungSD, BUC.KbBuchungStatusCode, Schuldner_BaInstitutionID = POS.BaInstitutionID, Schuldner_BaPersonID = POS.DebitorBaPersonID, VerwPeriodeVon, VerwPeriodeBis, Abgetreten = CASE WHEN BUC.BgKategorieCode = 3 OR BUC.VerwaltungSD = 1 THEN 1 ELSE 0 END
    ...
    FROM #Buchungen BUC
      ...
      LEFT OUTER JOIN dbo.BgAuszahlungPersonTermin BAT WITH (READUNCOMMITTED) ON BAP.BgAuszahlungPersonID = BAT.BgAuszahlungPersonID
    WHERE ABS(BUC.BetragBrutto) > $0.00
    GROUP BY BUC.BgPositionID, BUC.BaPersonID_Teil, BUC.Kostenstelle, BUC.BgPositionsartID, BUC.Name, BUC.BgKostenartID, BUC.AuszahlGruppeID, BUC.Einnahme, COALESCE(BAT.Datum, POS.FaelligAm, @FirstDayInMonth), /*BAP.Zahlungsgrund, */BUC.VerwaltungSD, BUC.KbBuchungStatusCode, POS.BaInstitutionID, POS.DebitorBaPersonID, VerwPeriodeVon, VerwPeriodeBis, CASE WHEN BUC.BgKategorieCode = 3 OR BUC.VerwaltungSD = 1 THEN 1 ELSE 0 END
    ORDER BY BUC.BaPersonID_Teil, BUC.AuszahlGruppeID
...
/*
  ...
  */
  ...
  INSERT INTO @KbBuchungBrutto (KbPeriodeID, BgKostenartID, KbBuchungTypCode, BelegDatum, ValutaDatum, ErfassungsDatum, Betrag, [Text], KbBuchungStatusCode, UserID, BgBudgetID, Kostenstelle, BetragEffektiv, Hauptvorgang, Teilvorgang, Belegart, BgPositionID, FaLeistungID, BgSplittingArtCode, Weiterverrechenbar, Quoting, Abgetreten, Einnahme)
    ...
    GROUP BY BKA.BgKostenartID, Termin, BKA.Name, KbBuchungStatusCode, TMP.Abgetreten, BKA.Hauptvorgang, BKA.Teilvorgang, BKA.HauptvorgangAbtretung, BKA.TeilvorgangAbtretung, BKA.Belegart, CASE Einnahme WHEN 1 THEN TMP.BgPositionID END, BKA.BgSplittingArtCode, BKA.Weiterverrechenbar, BKA.Quoting, IsNull(TMP.Abgetreten,0), Einnahme
  ...
  INSERT INTO @KbBuchungBruttoPerson (KbBuchungBruttoID, BgPositionID, BaPersonID, Schuldner_BaInstitutionID, Schuldner_BaPersonID, Buchungstext, Betrag, VerwPeriodeVon, VerwPeriodeBis)
    ...
    FROM   #BruttoAuszahlungen    AUZ
      ...
      INNER JOIN @KbBuchungBrutto KBN ON KBN.BgKostenartID = AUZ.BgKostenartID AND AUZ.Termin = KBN.ValutaDatum
                                           AND KBN.Hauptvorgang = CASE AUZ.Abgetreten WHEN 1 THEN BKA.HauptvorgangAbtretung ELSE BKA.Hauptvorgang END
                                           AND KBN.Teilvorgang  = CASE AUZ.Abgetreten WHEN 1 THEN BKA.TeilvorgangAbtretung  ELSE BKA.Teilvorgang  END
