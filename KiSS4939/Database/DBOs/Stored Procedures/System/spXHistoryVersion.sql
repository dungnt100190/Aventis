...
FROM [' + @TableName + ']'
  ...
  SELECT TOP 1
     @HasIdentityColumn = CONVERT(BIT, COLUMNPROPERTY(OBJECT_ID(@TableName),COL.[Name],'IsIdentity'))
  FROM syscolumns           AS COL
  LEFT JOIN systypes        AS TYP ON TYP.xtype = COL.xtype
  LEFT JOIN sysobjects      AS OBJ ON OBJ.[ID] = COL.[ID]
  LEFT JOIN syscomments     AS COM ON COM.[ID] = COL.[ID]
  WHERE OBJ.[Name] = @TableName
  ORDER BY COLUMNPROPERTY(OBJECT_ID(@TableName),COL.[Name],'IsIdentity') DESC


  -- create trigger as sql statement
...
AS BEGIN
  SET NOCOUNT ON
  ' +
  CASE WHEN @HasIdentityColumn = 1 THEN 'SET IDENTITY_INSERT [Hist_' + @TableName + '] ON' ELSE '' END
  + '
  DECLARE @VerID     INT
  ...
  CASE WHEN @HasIdentityColumn = 1 THEN '  SET IDENTITY_INSERT [Hist_' + @TableName + '] OFF' ELSE '' END +
  '
  SET NOCOUNT OFF
END'

  DECLARE @sqlPK_IS_NULL  varchar(4000),
          @sqlIsNull_PK   varchar(4000),
          @sql5           varchar(4000),
          @sqlFieldList   varchar(4000),
          @sqlCompText    varchar(4000),
          @sqlCompVARIANT varchar(4000)
