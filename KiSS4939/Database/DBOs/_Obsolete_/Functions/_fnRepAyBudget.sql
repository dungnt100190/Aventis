-- used in 6012, 6013
CREATE FUNCTION dbo.fnRepAyBudget
 (@BgBudgetID int,
  @RefDate    datetime)
...
AS BEGIN
  ...
  INSERT INTO @BgPosition
    SELECT BPO.BgPositionID, BPO.BgPositionCode, SPT.ShPositionTypCode, BPO.ShPositionTypID,
    ...
    FROM vwBgPosition             BPO
      LEFT  JOIN AyPositionTyp    SPT ON SPT.ShPositionTypID = BPO.ShPositionTypID
    ...
    GROUP BY BPO.BgPositionID, BPO.BgPositionCode, BPO.ShPositionTypID, SPT.ShPositionTypCode, STZ.ShZahlungsmodusID
  ...
  INSERT INTO @BgPosition (BgPositionID, BgPositionCode, ShZahlungsmodusID, ShSpezkontoID, ShPositionTypID, Bezeichnung, Betrag, FlKostenartID)
    SELECT SEZ.ShEinzelzahlungID, 100, SZM.ShZahlungsmodusID, SEZ.ShSpezkontoID, SEZ.ShPositionTypID,
           IsNull(SEZ.NameEinzelzahlung, SPT.Name), SEZ.Betrag, SEZ.FlKostenartID
    FROM ShEinzelzahlung          SEZ
      ...
      LEFT  JOIN AyPositionTyp    SPT ON SPT.ShPositionTypID   = SEZ.ShPositionTypID
  ...
  INSERT INTO @budget
    SELECT BPO.BgPositionCode, XPC.Text,
      ...
      CASE BPO.BgPositionCode
        WHEN 1   THEN 'Einnahmen, ' + CASE WHEN BPO.Dritte = 1 THEN 'von AK' ELSE 'von Klient' END + ' verwaltet'
