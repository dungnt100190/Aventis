...
GO
/*===============================================================================================
  $Archive: /KiSS4/Database/DBOs/Functions/Sozialhilfe/ZOL/fnWhGetFinanzplanIndent.sql $
  $Author: Tabegglen $
  $Modtime: 09.10.14$
  $Revision: 2 $
=================================================================================================
...
BEGIN
  ...
  WHERE BgKategorieCode IN (1, 2, 4) AND BgPositionsartCode <> 107
  ...
  INSERT INTO @Budget (Style, Parent_ID, WhGrundbedarfTypCode, BgKategorieCode, BgGruppeCode,
    ...
    SELECT 10, BPA.BgKategorieCode, BFP.WhGrundbedarfTypCode, BPA.BgKategorieCode, BPA.BgGruppeCode2,
      ...
      Info = CASE
			         WHEN BPA.BgGruppeCode2 = 3102 THEN
                 (SELECT ', Alimenteninkasso: ' + LTrim(STR(SUM(POS.BetragFinanzplan), 19, 2))
                  FROM dbo.vwBgPosition POS
                  INNER JOIN BgPositionsart POA ON POA.BgPositionsartID = POS.BgPositionsartID
                  WHERE POS.BgBudgetID = @BgBudgetID AND POA.BgPositionsartCode = 107
...
-- BSS! im SQL-Server 2000 gab das Coalesce Probleme!
  ...
  INSERT INTO @Budget (Style, Parent_ID, WhGrundbedarfTypCode, BgKategorieCode, BgGruppeCode,
    ...
    SELECT 
      ...
      Bezeichnung= 'Kürzung: ' + MAX(SPZ.NameSpezkonto), IsNull(SUM(BPO.BetragFinanzplan), $0.00), 1000, Info = ', Kürzung'
