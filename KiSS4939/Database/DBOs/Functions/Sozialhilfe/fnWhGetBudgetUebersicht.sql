...
BEGIN
  ...
  INSERT INTO @OUTPUT (Rec_ID, Parent_ID, Style, Bezeichnung) VALUES (-10, -1, 2, 'Leistungen des Sozialdienstes');
  SET @BetragSumme = 0;
  ...
  INSERT INTO @OUTPUT (Parent_ID, Style, Bezeichnung, Betrag) VALUES (-10, 3, 'Abzüglich Kürzungen', IsNull(0 - @Betrag, 0));
  SET @BetragSumme = @BetragSumme - IsNull(@Betrag, 0);
  ...
  SELECT @Betrag = SUM(Betrag) FROM @BgBudget WHERE BgKategorieCode = 100 AND Style = 85 AND BgSpezkontoID IS NULL;
  ...
  INSERT INTO @OUTPUT (Rec_ID, Parent_ID, Style, Bezeichnung) VALUES (-20, -1, 2, 'Zahlungen des Sozialdienstes');
  ...
  INSERT INTO @OUTPUT (Parent_ID, Style, Bezeichnung, Betrag)
    SELECT -20, 3, 'Abzüge für ' + CASE WHEN XLC.text LIKE 'Kürzungen' THEN XLC.text ELSE LEFT(XLC.text, LEN(XLC.text) - 1) + 'i' END, IsNull(SUM(-BPO.BetragRechnung), 0)
    FROM dbo.XLOVCode        XLC WITH (READUNCOMMITTED) 
      LEFT  JOIN (SELECT SUM(BetragRechnung) AS BetragRechnung, SSK.BgSpezkontoTypCode
                  FROM @BgBudget            TMP
  ...
  INSERT INTO @OUTPUT (Rec_ID, Parent_ID, Style, Bezeichnung, Betrag) VALUES (-21, -20, 3, 'Von SD verwaltete Einnahmen', IsNull(@Betrag, 0));
  SET @BetragSumme = @BetragSumme + IsNull(@Betrag, 0);
  ...
  INSERT INTO @OUTPUT (Parent_ID, Style, Bezeichnung, Betrag) VALUES (-20, 3, 'Zahlungen aus Vorabzugs-, Abzahlungs- und Ausgabekonti', IsNull(@Betrag, 0));
  SET @BetragSumme = @BetragSumme + IsNull(@Betrag, 0);
  ...
  INSERT INTO @OUTPUT (Parent_ID, Style, Bezeichnung) VALUES (-1, 2, '');
  INSERT INTO @OUTPUT (Rec_ID, Parent_ID, Style, Bezeichnung) VALUES (-30, -1, 2, 'Zahlungen des Sozialdienstes an Klient/in');
  ...
  SELECT @Betrag = SUM(BetragRechnung) FROM @BgBudget WHERE BgKategorieCode IN (2, 100, 101) AND BgAuszahlungID IS NOT NULL AND Dritte = 1;
