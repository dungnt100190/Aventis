...
AS BEGIN
  ...
  DECLARE @FaProzessCode     INT,
          ...
          @DezemberAusweiten BIT;
  
  -- BFSLeistungsartCode in Prozesscode umwandeln.
  SET @FaProzessCode = dbo.fnBFSLeistungsartTOProzess(@BFSLeistungsartCode);

  SET @DezemberAusweiten = CASE @Monat 
                             WHEN 12 THEN 1 
                             ELSE 0 
                           END;
  
  -- Zeitraum erweitern für Dezember, da Dezemberbudgets im November, Januarbudgets im Januar ausbezahlt werden
  IF (@DezemberAusweiten = 1)
  BEGIN
    SET @BuchungenVon = dbo.fnDateSerial(@Jahr,    11, 20);
    SET @BuchungenBis = dbo.fnDateSerial(@Jahr + 1, 1, 10);
  END
  ELSE BEGIN
  ...
  WHERE LEI.BaPersonID = @AntragstellerID 
    ...
    AND (@DezemberAusweiten = 0 OR BDG.Monat = @Monat AND BDG.Jahr = @Jahr)
    AND CASE 
