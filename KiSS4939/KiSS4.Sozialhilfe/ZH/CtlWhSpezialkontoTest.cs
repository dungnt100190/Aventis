...
{
    public class CtlWhSpezialkontoTest : KiSS4.Gui.KissUserControl
    {
        #region Fields
        ...
        public CtlWhSpezialkontoTest(BgSpezkontoTyp BgSpezkontoTypCode, int faLeistung, object InitName, object InitStartSaldo, object InitBelastung)
        ...
        public CtlWhSpezialkontoTest(Image titelImage, string titelText, BgSpezkontoTyp BgSpezkontoTypCode, int faLeistung)
            : this()
        {
        ...
        public CtlWhSpezialkontoTest()
        {
            this.InitializeComponent();
            //Testcode
            this.FaLeistungID = 7030228;
            this.picTitel.Image = null;
            this.BgSpezkontoTypCode = BgSpezkontoTyp.Abzahlungskonto;

            // Kostenarten
            qryBgKostenart.Fill(this.FaLeistungID, (int)BgSpezkontoTypCode);
            edtBgKostenartID.LoadQuery(qryBgKostenart, "BgKostenartID", "NrName");

            lblTitel.Text = "Spezialkonto Test";
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(CtlWhSpezialkontoTest));
            DevExpress.Utils.SerializableAppearanceObject serializableAppearanceObject1 = new DevExpress.Utils.SerializableAppearanceObject();
            DevExpress.Utils.SerializableAppearanceObject serializableAppearanceObject2 = new DevExpress.Utils.SerializableAppearanceObject();
            this.components = new System.ComponentModel.Container();
            this.panHeader = new System.Windows.Forms.Panel();
            this.edtInaktiv = new KiSS4.Gui.KissCheckEdit();
            this.qryBgSpezkonto = new KiSS4.DB.SqlQuery();
            ...
            this.Name = "CtlWhSpezialkontoTest";
            this.Size = new System.Drawing.Size(680, 586);
            this.Load += new System.EventHandler(this.CtlWhSpezialkontoTest_Load);
        ...
        private void CtlWhSpezialkontoTest_Load(object sender, System.EventArgs e)
        {
            // Positionsarten
        ...
        private void edtBgKostenartID_EditValueChanged(object sender, System.EventArgs e)
        {
            ...
            if (!qryBgSpezkonto.IsFilling && !qryBgPosition.IsFilling && !DBUtil.IsEmpty(edtBgKostenartID.EditValue))
            {
                object quoting = DBUtil.ExecuteScalarSQL("SELECT Quoting FROM BgKostenart WHERE BgKostenartID = {0}", edtBgKostenartID.EditValue);
                qryBgSpezkonto["BgKostenartID"] = edtBgKostenartID.EditValue;
                qryBgSpezkonto["Quoting"] = quoting;
                if (!DBUtil.IsEmpty(quoting) && (bool)quoting)
                {
        ...
        private void edtBgPositionsartID_EditValueChanged(object sender, EventArgs e)
        {
            if (!qryBgSpezkonto.IsFilling && !qryBgPosition.IsFilling && !DBUtil.IsEmpty(edtBgKostenartID.EditValue))
            {
                if (!DBUtil.IsEmpty(((SqlQuery)edtBgKostenartID.Properties.DataSource)["Quoting"]) && (bool)((SqlQuery)edtBgKostenartID.Properties.DataSource)["Quoting"])
                    // Bei gequoteten LAs darf man kein Person auswählen
                    qryBgSpezkonto["BaPersonID"] = null;
            }
        ...
        private void qryBgSpezkonto_AfterDelete(object sender, System.EventArgs e)
        {
            ...
            if (qryBgSpezkonto.Count == 0) DisplayBewegungen();
        }
        ...
        private void qryBgSpezkonto_BeforeDelete(object sender, System.EventArgs e)
        {
            ...
            try
            {
                // Dieses SQL-Statement sollte mit dem zweiten Teil des SQL-Statements in SqlQuery_BeforeDelete übereinstimmen
                DBUtil.ExecSQL(@"
                    DELETE BPO
                    FROM BgPosition        BPO
            ...
            if (Session.Transaction == null) throw new KissCancelException();
        }
        ...
        private void qryBgSpezkonto_BeforeDeleteQuestion(object sender, System.EventArgs e)
        {
            ...
            throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "BereitsGebucht", "Auf dieses Konto wurde bereits gebucht und kann deshalb nicht mehr gelöscht werden", KissMsgCode.MsgInfo));
        }
        ...
        private void qryBgSpezkonto_BeforePost(object sender, System.EventArgs e)
        {
            ...
            {
                throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "DatBisVorDatVon", "'gültig bis' darf nicht vor 'gültig von' sein.", KissMsgCode.MsgInfo), editDatumVonMonat);
            }
            ...
            if (qryBgSpezkonto.Row.RowState != System.Data.DataRowState.Added && qryBgSpezkonto.ColumnModified("Startsaldo"))
            {
                ...
                if (Saldo < 0)
                {
                    ...
                    throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "MinStartsaldo", "Der Startsaldo muss mindestens Fr. {0:n2} betragen", KissMsgCode.MsgInfo, Saldo), this.edtStartSaldo);
                }
            ...
            switch (BgSpezkontoTypCode)
            {
                case BgSpezkontoTyp.Vorabzugkonto:
                    ...
                    if (qryBgSpezkonto.ColumnModified("Startsaldo"))
                    {
                        ...
                        if (null == o)
                            throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "KompetenzNichtEingerichtet", "Die Kompetenz 'Sozialhilfe: Monatsbudget - max. Startsaldo eines Vorabzugskontos' muss zuerst für Sie eingerichtet werden.", KissMsgCode.MsgInfo));

                        decimal maxStartSaldo = Convert.ToDecimal(o);
                        if (this.edtStartSaldo.Value > maxStartSaldo)
                            throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "KeineKompetenzStartsaldo", "Sie haben nicht die Kompetenz, einen Startsaldo von {0:n2} einzugeben.\r\nIhre Kompetenz liegt bei {1:n2}", KissMsgCode.MsgInfo, this.edtStartSaldo.Value, maxStartSaldo));
                    }
                    break;
                ...
                case BgSpezkontoTyp.Abzahlungskonto:
                    ...
                    {
                        throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "EinzelzahlungExistiertBereits", "Rückerstattung ohne Einzelzahlung darf nicht mehr verändert werden.\r\nEs existiert bereits eine Einzelzahlung auf diesem Konto", KissMsgCode.MsgInfo));
                    }
                    ...
                    if (qryBgSpezkonto.Row.RowState == System.Data.DataRowState.Added)
                    {
                        ...
                        if (!DBUtil.IsEmpty(o))
                            throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "MonatsbudgetGesperrt", "'gültig von' ist nicht zulässig, da freigegebene oder gesperrte Monatsbudgets in diesem oder späteren Monat vorhanden sind.", KissMsgCode.MsgInfo), this.editDatumVonMonat);
                        ...
                        if ((int)o > 0)
                            throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "AbzahlungGemacht", "'gültig von' ist nicht zulässig. Es sind {0} Abzahlungen vor diesem Monat gemacht worden", KissMsgCode.MsgInfo, o), this.editDatumVonMonat);
                    }

                    if (Convert.ToDecimal(colBetragBelastung.SummaryItem.SummaryValue) > Convert.ToDecimal(qryBgSpezkonto["StartSaldo"]))
                        throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "SummeEinzUebersteigtAbzahlungsziel", "Die Summe aller Einzelzahlungen ({0:n2}) übersteigt das Abzahlungsziel ({1:n2}))", KissMsgCode.MsgInfo, colBetragBelastung.SummaryItem.SummaryValue, qryBgSpezkonto["StartSaldo"]), this.edtStartSaldo);

                    if (Convert.ToDecimal(colBetragGutschriftEffektiv.SummaryItem.SummaryValue) > Convert.ToDecimal(qryBgSpezkonto["StartSaldo"]))
                        throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "SummeAbzUebersteigtAbzahlungsziel", "Die Summe aller Abzahlungen ({0:n2}) übersteigt das Abzahlungsziel ({1:n2}))", KissMsgCode.MsgInfo, colBetragGutschriftEffektiv.SummaryItem.SummaryValue, qryBgSpezkonto["StartSaldo"]), this.edtStartSaldo);

                    if (DBUtil.IsEmpty(qryBgSpezkonto["BetragProMonat"]) || Convert.ToDecimal(qryBgSpezkonto["BetragProMonat"]) <= (decimal)0)
                        throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "BetragMonatGroesserNull", "Der monatliche Betrag muss > 0.00 sein.", KissMsgCode.MsgInfo), this.edtBetragProMonat);

                    if (!DBUtil.IsEmpty(qryBgSpezkonto["DatumBis"]))
                    {
                        // Falls DatumBis erfasst: die nötige Anzahl Raten muss kleiner sein als die Anzahl Monate zwischen
                        // 'gültig von' und 'gültig bis'
                        // inaktive Monate verlängern die benötigte Zeitspanne
                        int payments = GetPaymentTotalMonths((decimal)qryBgSpezkonto["StartSaldo"], (decimal)qryBgSpezkonto["BetragProMonat"], (int)qryBgSpezkonto["BgSpezkontoID"]);
                        ...
                        if (payments > months)
                            throw new KissInfoException(KissMsg.GetMLMessage("CtlWhSpezialkontoTest", "AbzahlungenNoetig", "Es sind {0} Abzahlungen nötig, zwischen 'gültig von' und 'gültig bis' liegen aber nur {1} Monate.", KissMsgCode.MsgInfo, payments, months));
                    }
                    break;
            }
            ...
            if (BgSpezkontoTypCode == BgSpezkontoTyp.Ausgabekonto) // auch für Verrechnungskonto überprüfen?
        ...
        private void CalcDatumBis()
        {
            ...
            {
                /*              object datumBis = DBUtil.ExecuteScalarSQL("SELECT dbo.[fnWhBudget_GetSpezkontoEnddatumTest] ({0}, {1}, {2}, {3}, {4})", qryBgSpezkonto["BgSpezkontoID"],
                ...
                */
                ...
                object datumBis = DBUtil.ExecuteScalarSQL("Exec dbo.[spWhBudget_GetSpezkontoEnddatumTest] {0}, {1}, {2}, {3}, {4}", qryBgSpezkonto["BgSpezkontoID"],
                                                               new DateTime((int)qryBgSpezkonto["DatumVonJahr"], (int)qryBgSpezkonto["DatumVonMonat"], 1),
                                                               qryBgSpezkonto["StartSaldo"],
                                                               qryBgSpezkonto["BetragProMonat"],
                                                               qryBgSpezkonto["Inaktiv"]);
        ...
        private int GetPaymentTotalMonths(decimal startSaldo, decimal betragProMonat, int bgSpezkontoID)
        {
            // Zeitdauer = Total/monatlicher Betrag
            int payments = (int)Math.Floor((double)(startSaldo / betragProMonat));
            // Restbetrag wird in einem zusätzlichen Monat bezahlt
            decimal remainder = startSaldo % betragProMonat;
            if (remainder > 0)
                payments++;

            if (bgSpezkontoTypCode == BgSpezkontoTyp.Abzahlungskonto)
            {
                // Die inaktiven Monate verlängern die Dauer der Abzahlung
                payments += (int)DBUtil.ExecuteScalarSQL(@"
                  SELECT Count(*)
                  FROM dbo.BgPosition       BPO
                    INNER JOIN dbo.BgBudget BUD ON BUD.BgBudgetID = BPO.BgBudgetID AND BUD.Masterbudget = 0 AND BUD.BgBewilligungStatusCode >= 5 /*grün*/
        ...
        {
            qryBgSpezkonto.ApplyUserRights(string.Format("CtlWhSpezialkontoTest_{0}", this.BgSpezkontoTypCode));
        ...
        private void SetEditMode()
        {
            ...
            bool verrechnungBeglichen = verbuchteBuchungenVorhanden && (decimal)qryBgSpezkonto["Saldo"] == 0m;
            bool quoting;
            ...
            editDatumBisMonat.EditMode = verrechnungBeglichen ? EditModeType.ReadOnly : EditModeType.Normal;
            editDatumBisJahr.EditMode = verrechnungBeglichen ? EditModeType.ReadOnly : EditModeType.Normal;
