...
{
    public class CtlBgSilAHVBeitrag : KiSS4.Gui.KissUserControl
    {
        #region Fields

        #region Private Constant/Read-Only Fields

        const LOV.BgGruppeCode BgGruppeCode = LOV.BgGruppeCode.AHV_Beitraege;
        ...
        public CtlBgSilAHVBeitrag(Image titelImage, string titelText, int bgBudgetID)
            : this()
        {
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(CtlBgSilAHVBeitrag));
            ...
            this.btnBewilligung.TabIndex = 56;
        ...
        {
            this.qryBgPosition[DBO.BgPosition.BgBudgetID] = this.BgBudgetID;
            this.qryBgPosition[DBO.BgPosition.BgKategorieCode] = 2;

            if (bAnpassen)
            {
                this.qryBgPosition[DBO.BgPosition.DatumVon] = this.AnpassenVon;
            }
            else
            {
                this.qryBgPosition[DBO.BgPosition.DatumVon] = qryBgFinanzplan["FinanzplanVon"];
            }
        ...
        {
            if ((int)qryBgPosition[DBO.BgPosition.BgBewilligungStatusCode] >= (int)BgBewilligungStatus.Erteilt
                || 0 < (int)DBUtil.ExecuteScalarSQL("SELECT COUNT(*) FROM BgPosition WHERE BgBudgetID = {0} AND BgPositionID_CopyOf = {1}",
                    qryBgPosition[DBO.BgPosition.BgBudgetID], qryBgPosition[DBO.BgPosition.BgPositionID]))
            {
                throw new KissInfoException(
                    KissMsg.GetMLMessage("CtlBgSilAHVBeitrag",
                                         "NichtLoeschen",
        ...
        private void qryBgPosition_BeforeInsert(object sender, System.EventArgs e)
        {
            ...
            if (!bAnpassen && (int)qryBgFinanzplan[DBO.BgPosition.BgBewilligungStatusCode] == (int)BgBewilligungStatus.Erteilt)
            {
        ...
        private void qryBgPosition_BeforePost(object sender, System.EventArgs e)
        {
            ...
            if (!DBUtil.IsEmpty(edtDatumVon.EditValue))
            {
                ...
                {
                    throw new KissInfoException(KissMsg.GetMLMessage("CtlBgSilAHVBeitrag", "GeplantInnerhFinanzpl", "Geplant von muss innherhalb von Zeitspanne des Finanzplanes liegen ({0:d} - {1:d}).", KissMsgCode.MsgInfo, qryBgFinanzplan["FinanzPlanVon"], qryBgFinanzplan["FinanzPlanBis"]));
                }
                ...
                if (!DBUtil.IsEmpty(edtDatumBis.EditValue))
                {
                    ...
                    {
                        throw new KissInfoException(KissMsg.GetMLMessage("CtlBgSilAHVBeitrag", "GeplantVonZuGross", "Geplant von muss kleiner sein als geplant bis."));
                    }
                }
            }

            if ((DBUtil.IsEmpty(qryBgPosition[DBO.BgPosition.Buchungstext]) || qryBgPosition.ColumnModified(DBO.BgPosition.BgPositionsartID))
                && qryBgPositionsart.Find(string.Format("BgPositionsartID = {0}", qryBgPosition["BgPositionsartID"])))
            {
                qryBgPosition[DBO.BgPosition.Buchungstext] = qryBgPositionsart[DBO.BgPositionsart.Name];
            }
            ...
            try
            {
                if (bAnpassen)
                {
                    ...
                    if (!DBUtil.IsEmpty(qryBgPosition[DBO.BgPosition.BgPositionID]))
                    {
        ...
        private void qryBgPosition_PositionChanged(object sender, System.EventArgs e)
        {
            ...
            try
            {
                BgBewilligungStatusCode = (int)qryBgPosition[DBO.BgPosition.BgBewilligungStatusCode];
            }
            catch
            {
