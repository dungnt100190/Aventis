...
namespace KiSS4.DesignerHost
{
	...
	public class MyEventBindingService : IEventBindingService 
	{
		...
		public MyEventBindingService(IServiceProvider provider) 
		{
		...
		private class EventPropertyDescriptor : PropertyDescriptor 
		{
			...
			private MyEventBindingService _eventSvc;
			private TypeConverter       _converter;

			///     Creates a new EventPropertyDescriptor.
			internal EventPropertyDescriptor(EventDescriptor eventDesc, MyEventBindingService eventSvc) : base(eventDesc, null) 
			{
			...
			{
				get { return _eventDesc.ComponentType; }
			}
			...
			{
				get { return _eventDesc.EventType; }
			}
			...
			public override object GetValue(object component) 
			{
				...
				if (site == null) 
				{
                    if (component is DevExpress.Utils.AppearanceObject) return null;
                    
                    throw new InvalidOperationException( string.Format("There is no site for component '{0}'.", component));
				}
			...
			public override void SetValue(object component, object value) 
			{
				...
				if (IsReadOnly) 
				{
					throw new InvalidOperationException(string.Format("Tried to set a read only event for component '{0}' and value '{1}'.", component, value));
				}
                
				if (value != null && !(value is string)) 
				{
                    throw new ArgumentException(string.Format("Cannot set value '{0}' of component '{1}'.", value, component));
				}
