...
namespace KiSS4.DesignerHost
{
	...
	public class MyMenuCommandService : System.ComponentModel.Design.IMenuCommandService
	{
		...
		public MyMenuCommandService(MyDesignSurface designSurfase)
		{
		...
		{
			ISelectionService ss = designSurfase.GetService(typeof(ISelectionService)) as ISelectionService;
            IDesignerHost dh = designSurfase.GetService(typeof(IDesignerHost)) as IDesignerHost;
			...
			//
            if ((lastSelection != null) && (lastSelection != ss.PrimarySelection) && dh.GetDesigner(lastSelection)!=null && dh.GetDesigner(lastSelection).Verbs != null)
                foreach (DesignerVerb verb in dh.GetDesigner(lastSelection).Verbs)
					RemoveLocalVerb(verb);

			// Update the local verbs for the new selection, if it is indeed new.
            if (lastSelection != ss.PrimarySelection && dh.GetDesigner(ss.PrimarySelection as IComponent).Verbs!=null)
                foreach (DesignerVerb verb in dh.GetDesigner(ss.PrimarySelection as IComponent).Verbs)
					AddLocalVerb(verb);
			...
			if (cm != null) 
			{
				...
				if (ps == null)
                    ps = designSurfase.View as Control;

				if (ps != null) 
				{
					Point s = new Point(0, 0);
					s = ps.PointToScreen(new Point(0, 0));
		...
		public System.ComponentModel.Design.DesignerVerbCollection Verbs
		{
			get
			{
                ...
                // do we have any verbs to add?
				if (ss.SelectionCount > 0)
				{
					if (dh.GetDesigner(ss.PrimarySelection as IComponent) != null && dh.GetDesigner(ss.PrimarySelection as IComponent).Verbs != null)
					{
						foreach (DesignerVerb verb in dh.GetDesigner(ss.PrimarySelection as IComponent).Verbs)
		...
		private void menuItem_Click(object sender, EventArgs e)
		{
            ...
            {
                KiSS4.DB.KissMsg.ShowError("MyMenuCommandService", "ErrorMenuItemClick", "Fehler beim Ausführen des Menubefehls", ex);
            }
		}
	}
}
