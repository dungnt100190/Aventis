...
namespace KiSS4.DesignerHost
{
    ...
    ///
    /// </copyright>
    internal class MySelectionService : ISelectionService
    {
        #region Fields
        ...
        private MySelectionItem primarySelection; // the primary selection
        ...
        internal MySelectionService(IDesignerHost host)
        {
            this.host = host;
        ...
        internal void Dispose()
        {
            ...
            MySelectionItem[] sels = new MySelectionItem[selectionsByComponent.Values.Count];
        ...
        object ISelectionService.PrimarySelection
        {
            get
            {
                if (primarySelection == null)
                {
                    ...
                    if (valueFound)
                    {
                        primarySelection = (MySelectionItem)selections.Value;
        ...
        {
            MySelectionItem sel = (MySelectionItem)selectionsByComponent[ce.Component];
        ...
        ICollection ISelectionService.GetSelectedComponents()
        {
            ...
            for (int i = 0; i < sels.Length; i++)
                objects[i] = ((MySelectionItem)sels[i]).Component;
        ...
        void ISelectionService.SetSelectedComponents(ICollection components, SelectionTypes selectionType)
        {
            ...
            if (!fToggle && !fControl)
            {
                ...
                {
                    MySelectionItem oldPrimary = primarySelection;
                    SetPrimarySelection((MySelectionItem)selectionsByComponent[firstSelection]);
                    if (oldPrimary != primarySelection)
                        fChanged = true;
                }
                else
                {
                    MySelectionItem[] selections = new MySelectionItem[selectionsByComponent.Values.Count];
                    selectionsByComponent.Values.CopyTo(selections, 0);

                    // Even with several hundred components this should be fairly fast
                    foreach (MySelectionItem item in selections)
                    {
                        bool remove = true;
            ...
            MySelectionItem primarySel = null;
            ...
            foreach (Component comp in components)
            {
                if (comp != null)
                {
                    MySelectionItem s = (MySelectionItem)selectionsByComponent[comp];

                    if (null == s)
                    {
                        s = new MySelectionItem(this, comp);
                        AddSelection(s);
        ...
        internal void SetPrimarySelection(MySelectionItem sel)
        {
        ...
        private void AddSelection(MySelectionItem sel)
        {
        ...
        private void RemoveSelection(MySelectionItem s)
        {
